...
class Parser():
    '''
    A simple parser class to match a given string in a file and return the
    associated values
    '''
    ...
    def extractKeyValues(self, key, dtype='float', patternToCapture = r".*",
        ...
        '''
        This function takes a key (a string) and a regular-expression pattern
        to match after the key
        @key: [Input] string to match
        @dtype: [Input] A string specifying the datatype of the values to be
        returned. Default: 'float'. Allowed values: 'int', 'float', 'complex',
        'bool'.
        @patternToCapture: [Input] A string containing the regular-expression
        to capture after the `key` string. Default: captures everything after
        `key` until end of line
        @groupId: The group Id to capture from the pattern matched using
        `patterToCapture`. Default: 0 (i.e., uses the entire string matched
        after the `key`)
        @dtypePattern: A regular expression that defines how to extract a
