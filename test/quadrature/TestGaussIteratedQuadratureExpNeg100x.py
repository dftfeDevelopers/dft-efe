...
@rfm.simple_test
class TestGaussIteratedQuadratureExpNeg100xBuildOnly(rfm.CompileOnlyRegressionTest):
    descr = '''Compile only test for Gauss Iterated quadrature rule in a unit cube'''\
            '''i using exp(-100x) as test function'''
    build_system = 'CMake'
    make_opts = ['TestGaussIteratedQuadratureExpNeg100x']
    sourcesdir = './src'
...
@rfm.simple_test
class TestGaussIteratedQuadratureExpNeg100xBuildAndRun(rfm.RegressionTest):
    descr = '''Compile and run test for Gauss Iterated quadrature rule in a unit cube'''\
            ''' using exp(-100x) as test function'''
    build_system = 'CMake'
    make_opts = ['TestGaussIteratedQuadratureExpNeg100x']
    executable = './TestGaussIteratedQuadratureExpNeg100x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'aggregate', 'slowOrFast': 'fast', 'arch': 'cpu',
                'serialOrParallel': 'parallel'}
