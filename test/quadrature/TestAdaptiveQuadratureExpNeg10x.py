...
@rfm.simple_test
class TestAdaptiveQuadratureExpNeg10xCompileOnly(rfm.CompileOnlyRegressionTest):
    descr = '''Compile only test for adaptive quadrature rule in a unit cube'''\
            ''' using exp(-10x) as test function'''
    build_system = 'CMake'
    make_opts = ['TestAdaptiveQuadratureExpNeg10x']
    sourcesdir = './src'
...
@rfm.simple_test
class TestAdaptiveQuadratureExpNeg10x(rfm.RegressionTest):
    descr = '''Compile and run test for adaptive quadrature rule in'''\
            ''' a unit cube using exp(-10x) as test function'''
    build_system = 'CMake'
    make_opts = ['TestAdaptiveQuadratureExpNeg10x']
    executable = './TestAdaptiveQuadratureExpNeg10x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'aggregate', 'slowOrFast': 'fast', 'arch': 'cpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        '''
        hasTestPassed = True
        msg = 'Adaptive quadrature integration for exp(-10x) in a unit cube failed'
        bmfilename = "benchmarkTestAdaptiveQuadratureExpNeg10x"
        outfilename = "outTestAdaptiveQuadratureExpNeg10x"
