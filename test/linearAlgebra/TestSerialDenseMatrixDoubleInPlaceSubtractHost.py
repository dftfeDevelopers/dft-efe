...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleInPlaceSubtractHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleInPlaceSubtractHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleInPlaceSubtractHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleInPlaceSubtractHost(rfm.RegressionTest):
    descr = '''Compile and run test for the -= operator of SerialDenseMatrix with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleInPlaceSubtractHost']
    executable = './TestSerialDenseMatrixDoubleInPlaceSubtractHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleInPlaceSubtractHost."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleInPlaceSubtractHost."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleInPlaceSubtractHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleInPlaceSubtractHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for the -= operator of SerialDenseMatrix with double datatype'''
    target_name = 'TestSerialDenseMatrixDoubleInPlaceSubtractHost'
    build_system = 'CMake'
