...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleCopyHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleCopyHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleCopyHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleCopyHost(rfm.RegressionTest):
    descr = '''Compile and run test for testing the copyTo and copyFrom function of SerialDenseMatrix with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleCopyHost']
    executable = './TestSerialDenseMatrixDoubleCopyHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleCopyHost."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleCopyHost."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleCopyHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleCopyHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for testing copyTo and copyFrom of SerialDenseMatrix with double datatype'''
    target_name = 'TestSerialDenseMatrixDoubleCopyHost'
    build_system = 'CMake'
