...
@rfm.simple_test
class BuildOnlyTestBlasLapackComplexDoubleGemmDevice(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestBlasLapackComplexDoubleGemmDevice using CMake'
    build_system = 'CMake'
    make_opts = ['TestBlasLapackComplexDoubleGemmDevice']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestBlasLapackComplexDoubleGemmDevice(rfm.RegressionTest):
    descr = '''Compile and run test  for checking double Gemm on device'''
    build_system = 'CMake'
    make_opts = ['TestBlasLapackComplexDoubleGemmDevice']
    executable = './TestBlasLapackComplexDoubleGemmDevice.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'gpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestBlasLapackComplexDoubleGemmDevice."
        msgThrownException = "Found exceptions in TestBlasLapackComplexDoubleGemmDevice."
        msgAssertFail = "Found assert fail(s) in TestBlasLapackComplexDoubleGemmDevice."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestBlasLapackComplexDoubleGemmDevice(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for checking double Gemm on device'''
    target_name = 'TestBlasLapackComplexDoubleGemmDevice'
    build_system = 'CMake'
