...
@rfm.simple_test
class BuildOnlyTestSerialVectorComplexDoubleNormHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialVectorComplexDoubleNormHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialVectorComplexDoubleNormHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialVectorComplexDoubleNormHost(rfm.RegressionTest):
    descr = '''Compile and run test for computing norms of SerialVector with complex double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialVectorComplexDoubleNormHost']
    executable = './TestSerialVectorComplexDoubleNormHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialVectorComplexDoubleNormHost."
        msgThrownException = "Found exceptions in TestSerialVectorComplexDoubleNormHost."
        msgAssertFail = "Found assert fail(s) in TestSerialVectorComplexDoubleNormHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialVectorComplexDoubleNormHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing norms of SerialVector with complex double datatype'''
    target_name = 'TestSerialVectorComplexDoubleNormHost'
    build_system = 'CMake'
