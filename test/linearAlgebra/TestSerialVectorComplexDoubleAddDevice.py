...
@rfm.simple_test
class BuildOnlyTestSerialVectorComplexDoubleAddDevice(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialVectorComplexDoubleAddDevice using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialVectorComplexDoubleAddDevice']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialVectorComplexDoubleAddDevice(rfm.RegressionTest):
    descr = '''Compile and run test for adding two SerialVectors with double datatype on Device'''
    build_system = 'CMake'
    make_opts = ['TestSerialVectorComplexDoubleAddDevice']
    executable = './TestSerialVectorComplexDoubleAddDevice.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'gpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialVectorComplexDoubleAddDevice."
        msgThrownException = "Found exceptions in TestSerialVectorComplexDoubleAddDevice."
        msgAssertFail = "Found assert fail(s) in TestSerialVectorComplexDoubleAddDevice."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialVectorComplexDoubleAddDevice(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for adding two SerialVectors with double datatype on Device'''
    target_name = 'TestSerialVectorComplexDoubleAddDevice'
    build_system = 'CMake'
