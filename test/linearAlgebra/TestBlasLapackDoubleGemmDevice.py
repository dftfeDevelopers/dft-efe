...
@rfm.simple_test
class BuildOnlyTestBlasLapackDoubleGemmDevice(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestBlasLapackDoubleGemmDevice using CMake'
    build_system = 'CMake'
    make_opts = ['TestBlasLapackDoubleGemmDevice']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestBlasLapackDoubleGemmDevice(rfm.RegressionTest):
    descr = '''Compile and run test  for checking double Gemm on device'''
    build_system = 'CMake'
    make_opts = ['TestBlasLapackDoubleGemmDevice']
    executable = './TestBlasLapackDoubleGemmDevice.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'gpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestBlasLapackDoubleGemmDevice."
        msgThrownException = "Found exceptions in TestBlasLapackDoubleGemmDevice."
        msgAssertFail = "Found assert fail(s) in TestBlasLapackDoubleGemmDevice."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestBlasLapackDoubleGemmDevice(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for checking double Gemm on device'''
    target_name = 'TestBlasLapackDoubleGemmDevice'
    build_system = 'CMake'
