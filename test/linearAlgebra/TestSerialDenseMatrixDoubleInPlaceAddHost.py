...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleInPlaceAddHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleInPlaceAddHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleInPlaceAddHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleInPlaceAddHost(rfm.RegressionTest):
    descr = '''Compile and run test for the += operator of SerialDenseMatrix with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleInPlaceAddHost']
    executable = './TestSerialDenseMatrixDoubleInPlaceAddHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleInPlaceAddHost."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleInPlaceAddHost."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleInPlaceAddHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleInPlaceAddHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for the += operator of SerialDenseMatrix with double datatype'''
    target_name = 'TestSerialDenseMatrixDoubleInPlaceAddHost'
    build_system = 'CMake'
