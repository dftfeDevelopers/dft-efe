...
@rfm.simple_test
class BuildOnlyTestParallelMultiVectorCholeskyGSHostComplexDouble(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestParallelMultiVectorCholeskyGSHostComplexDouble using CMake'
    build_system = 'CMake'
    make_opts = ['TestParallelMultiVectorCholeskyGSHostComplexDouble']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestParallelMultiVectorCholeskyGSHostComplexDouble(rfm.RegressionTest):
    descr = '''Compile and run test for computing norms of SerialMultiVector with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestParallelMultiVectorCholeskyGSHostComplexDouble']
    executable = './TestParallelMultiVectorCholeskyGSHostComplexDouble.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestParallelMultiVectorCholeskyGSHostComplexDouble."
        msgThrownException = "Found exceptions in TestParallelMultiVectorCholeskyGSHostComplexDouble."
        msgAssertFail = "Found assert fail(s) in TestParallelMultiVectorCholeskyGSHostComplexDouble."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestParallelMultiVectorCholeskyGSHostComplexDouble(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing norms of SerialMultiVector with double datatype'''
    target_name = 'TestParallelMultiVectorCholeskyGSHostComplexDouble'
    build_system = 'CMake'
