...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleConstructorHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleConstructorHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleConstructorHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleConstructorHost(rfm.RegressionTest):
    descr = '''Compile and run test for constructors of SerialDenseMatrix with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleConstructorHost']
    executable = './TestSerialDenseMatrixDoubleConstructorHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleConstructorHost."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleConstructorHost."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleConstructorHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleConstructorHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for constructors of of SerialDenseMatrix with double datatype'''
    target_name = 'TestSerialDenseMatrixDoubleConstructorHost'
    build_system = 'CMake'
