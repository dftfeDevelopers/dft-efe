...
@rfm.simple_test
class BuildOnlyTestParallelMultiVectorMultipassLowdinHostComplexDouble(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestParallelMultiVectorMultipassLowdinHostComplexDouble using CMake'
    build_system = 'CMake'
    make_opts = ['TestParallelMultiVectorMultipassLowdinHostComplexDouble']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestParallelMultiVectorMultipassLowdinHostComplexDouble(rfm.RegressionTest):
    descr = '''Compile and run test for computing norms of SerialMultiVector with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestParallelMultiVectorMultipassLowdinHostComplexDouble']
    executable = './TestParallelMultiVectorMultipassLowdinHostComplexDouble.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestParallelMultiVectorMultipassLowdinHostComplexDouble."
        msgThrownException = "Found exceptions in TestParallelMultiVectorMultipassLowdinHostComplexDouble."
        msgAssertFail = "Found assert fail(s) in TestParallelMultiVectorMultipassLowdinHostComplexDouble."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestParallelMultiVectorMultipassLowdinHostComplexDouble(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing norms of SerialMultiVector with double datatype'''
    target_name = 'TestParallelMultiVectorMultipassLowdinHostComplexDouble'
    build_system = 'CMake'
