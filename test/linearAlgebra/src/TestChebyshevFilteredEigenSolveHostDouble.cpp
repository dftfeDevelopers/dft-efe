...
void generateHermitianColMajorMatrix(const std::vector<ValueType> &colMajorB, 
                                    std::vector<TypeReal> &eigenValues, 
                                    int rank, 
                                    ...
                                    std::vector<ValueType> &colMajorA)
{
  ...
  std::vector<TypeReal> T(globalSize*globalSize, 0);
  dftefe::utils::mpi::MPIBarrier(utils::mpi::MPICommWorld);
  if(rank == 0)
  {
    for( global_size_type i = 0 ; i < globalSize ; i++ )
    {
      for( global_size_type j = 0 ; j < globalSize ; j++ )
      {
        *(Q.data() + j*globalSize + i) = static_cast<TypeReal>(std::rand()) / RAND_MAX;
      }
    }
  }
...
//Restriction : a<b<c
void  
getEigenValuesInRange(TypeReal a, TypeReal b, TypeReal c, global_size_type globalSize, bool isLowestEigenValueDegenerate, std::vector<TypeReal> &eigenValues)
{
  ...
  {
    TypeReal val = (b - a) * ((TypeReal)std::rand() / RAND_MAX ) + a;
  ...
  {
    TypeReal val = (c - b) * ((TypeReal)std::rand() / RAND_MAX ) + b;
    if(std::find(std::begin(eigenValues),std::end(eigenValues),val) == std::end(eigenValues)) {
        eigenValues.push_back(val);
        count++;
    }
...
int main()
{
  ...
  std::vector<TypeReal> eigenValues(0);
  ...
  std::vector<ValueType> diagonal(0), subDiagonal(0);
  lanczos->getTridiagonalMatrix(diagonal, subDiagonal);
  ValueType residual = subDiagonal[subDiagonal.size()-1];
  ...
  dftefe::utils::mpi::MPIBarrier(utils::mpi::MPICommWorld);
    ...
    for( global_size_type i = 0 ; i < eigenSubspaceGuess.locallyOwnedSize() ; i++ )
    {
      ...
      {
        *(eigenSubspaceGuess.data() + i*numWantedEigenValues + j) = static_cast<TypeReal>(std::rand()) / RAND_MAX;
      }
    }
  ...
  std::vector<TypeReal>             eigenValuesCHFSI(numWantedEigenValues);
