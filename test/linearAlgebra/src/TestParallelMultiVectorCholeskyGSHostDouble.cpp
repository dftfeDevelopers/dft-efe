...
int main()
{
  ...
  size_type numOwnedIndices = 2;
  size_type maxNumGhostIndices = 4;
  ...
  for(size_type j = 0 ; j < numVec ; j++)
  {
    ...
    for(size_type i = 0 ; i < vecSize ; i++)
    { 
      std::cout << *(X0X0TX.data()+i*numVec+j) << ",";
    }
    std::cout << "]\n";
  }
    }
  ...
  for(size_type i = 0; i < X0X0TX.localSize(); ++i)
   {
    for(size_type j = 0 ; j < X0X0TX.numVectors(); j++)
    {
      if(std::fabs(*(X0X0TX.data() + i*numComponents + j)- 
        *(X.data() + i*numComponents + j)) > 1e-12)
      {
