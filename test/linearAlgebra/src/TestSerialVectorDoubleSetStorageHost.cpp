...
main()
{
  ...
  std::shared_ptr<VectorDoubleHost> dVec2
    = std::make_shared<SerialVectorDoubleHost>(vSize, 0,queue);
  dVec2->setValues<Host>(*memStorage2);

  *dVec1 += *dVec2;
  const MemoryStorageDoubleHost & dVecAddedStorage = dVec1->getValues();
  ...
  for(dftefe::size_type i = 0; i < vSize; ++i)
  {
	...
	"Expected value: " + std::to_string(dVecStd1[i]+dVecStd2[i]) + 
	" obtained value: " + std::to_string(dVecAddedHostCopy[i]);
