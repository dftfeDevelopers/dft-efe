...
'''
TestLinearAlgebraBlas1
This test runs and checks if blaspp is functioning well. 

command to run run-only test (must run TestLinearAlgebraBuild.py to compile the executables)
reframe -C ../config/mysettings.py -c TestLinearAlgebraBlas1.py -n 'RunOnlyTest_*' -r

command to run build-and-run test (the compilation is done internally with the ReFrame's stage director)
reframe -C ../config/mysettings.py -c TestLinearAlgebraBlas1.py -n 'BuildAndRunTest_*' -r
'''
@rfm.simple_test
class RunOnlyTest_TestBlas1(rfm.RunOnlyRegressionTest):
    target_name = 'TestBlas1'
...
@rfm.simple_test
class BuildAndRunTest_TestVectorAggregate1(rfm.RegressionTest):
    target_name = 'TestBlas1'
    descr = 'A build and run test using CMake'
    build_system = 'CMake'
    make_opts = [target_name]
