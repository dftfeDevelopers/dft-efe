...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleSetValuesHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleSetValuesHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleSetValuesHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleSetValuesHost(rfm.RegressionTest):
    descr = '''Compile and run test for testing the setValues function of SerialDenseMatrix with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleSetValuesHost']
    executable = './TestSerialDenseMatrixDoubleSetValuesHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleSetValuesHost."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleSetValuesHost."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleSetValuesHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleSetValuesHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for testing setValues of SerialDenseMatrix with double datatype'''
    target_name = 'TestSerialDenseMatrixDoubleSetValuesHost'
    build_system = 'CMake'
