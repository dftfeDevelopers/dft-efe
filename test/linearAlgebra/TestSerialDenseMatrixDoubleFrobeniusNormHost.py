...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleFrobeniusNormHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleFrobeniusNormHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleFrobeniusNormHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleFrobeniusNormHost(rfm.RegressionTest):
    descr = '''Compile and run test for computing Frobenius norm of SerialDenseMatrix with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleFrobeniusNormHost']
    executable = './TestSerialDenseMatrixDoubleFrobeniusNormHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleFrobeniusNormHost."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleFrobeniusNormHost."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleFrobeniusNormHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleFrobeniusNormHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing Frobenius norm of SerialDenseMatrix with double datatype'''
    target_name = 'TestSerialDenseMatrixDoubleFrobeniusNormHost'
    build_system = 'CMake'
