...
@rfm.simple_test
class BuildOnlyTestLanczosEigenSolverHostDouble(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestLanczosEigenSolverHostDouble using CMake'
    build_system = 'CMake'
    make_opts = ['TestLanczosEigenSolverHostDouble']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestLanczosEigenSolverHostDouble(rfm.RegressionTest):
    descr = '''Compile and run test for computing norms of SerialMultiVector with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestLanczosEigenSolverHostDouble']
    executable = './TestLanczosEigenSolverHostDouble.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestLanczosEigenSolverHostDouble."
        msgThrownException = "Found exceptions in TestLanczosEigenSolverHostDouble."
        msgAssertFail = "Found assert fail(s) in TestLanczosEigenSolverHostDouble."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestLanczosEigenSolverHostDouble(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing norms of SerialMultiVector with double datatype'''
    target_name = 'TestLanczosEigenSolverHostDouble'
    build_system = 'CMake'
