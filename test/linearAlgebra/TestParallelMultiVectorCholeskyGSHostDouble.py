...
@rfm.simple_test
class BuildOnlyTestParallelMultiVectorCholeskyGSHostDouble(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestParallelMultiVectorCholeskyGSHostDouble using CMake'
    build_system = 'CMake'
    make_opts = ['TestParallelMultiVectorCholeskyGSHostDouble']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestParallelMultiVectorCholeskyGSHostDouble(rfm.RegressionTest):
    descr = '''Compile and run test for computing norms of SerialMultiVector with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestParallelMultiVectorCholeskyGSHostDouble']
    executable = './TestParallelMultiVectorCholeskyGSHostDouble.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestParallelMultiVectorCholeskyGSHostDouble."
        msgThrownException = "Found exceptions in TestParallelMultiVectorCholeskyGSHostDouble."
        msgAssertFail = "Found assert fail(s) in TestParallelMultiVectorCholeskyGSHostDouble."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestParallelMultiVectorCholeskyGSHostDouble(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing norms of SerialMultiVector with double datatype'''
    target_name = 'TestParallelMultiVectorCholeskyGSHostDouble'
    build_system = 'CMake'
