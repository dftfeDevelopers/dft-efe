...
@rfm.simple_test
class BuildOnlyTestSerialMultiVectorDoubleNormHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialMultiVectorDoubleNormHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialMultiVectorDoubleNormHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialMultiVectorDoubleNormHost(rfm.RegressionTest):
    descr = '''Compile and run test for computing norms of SerialMultiVector with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialMultiVectorDoubleNormHost']
    executable = './TestSerialMultiVectorDoubleNormHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialMultiVectorDoubleNormHost."
        msgThrownException = "Found exceptions in TestSerialMultiVectorDoubleNormHost."
        msgAssertFail = "Found assert fail(s) in TestSerialMultiVectorDoubleNormHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialMultiVectorDoubleNormHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing norms of SerialMultiVector with double datatype'''
    target_name = 'TestSerialMultiVectorDoubleNormHost'
    build_system = 'CMake'
