...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleCopyDevice(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleCopyDevice using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleCopyDevice']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleCopyDevice(rfm.RegressionTest):
    descr = '''Compile and run test for testing the copyTo and copyFrom function of SerialDenseMatrix with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleCopyDevice']
    executable = './TestSerialDenseMatrixDoubleCopyDevice.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'gpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleCopyDevice."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleCopyDevice."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleCopyDevice."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleCopyDevice(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for testing copyTo and copyFrom of SerialDenseMatrix with double datatype'''
    target_name = 'TestSerialDenseMatrixDoubleCopyDevice'
    build_system = 'CMake'
