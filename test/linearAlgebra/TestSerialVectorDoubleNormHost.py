...
@rfm.simple_test
class BuildOnlyTestSerialVectorDoubleNormHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialVectorDoubleNormHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialVectorDoubleNormHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialVectorDoubleNormHost(rfm.RegressionTest):
    descr = '''Compile and run test for computing norms of SerialVector with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialVectorDoubleNormHost']
    executable = './TestSerialVectorDoubleNormHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialVectorDoubleNormHost."
        msgThrownException = "Found exceptions in TestSerialVectorDoubleNormHost."
        msgAssertFail = "Found assert fail(s) in TestSerialVectorDoubleNormHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialVectorDoubleNormHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing norms of SerialVector with double datatype'''
    target_name = 'TestSerialVectorDoubleNormHost'
    build_system = 'CMake'
