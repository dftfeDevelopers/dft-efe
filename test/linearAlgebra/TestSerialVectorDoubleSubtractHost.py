...
@rfm.simple_test
class BuildOnlyTestSerialVectorDoubleSubtractHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialVectorDoubleSubtractHost using CMake'
    build_system = 'CMake'
...
class BuildAndRunTestSerialVectorDoubleSubtractHost(rfm.RegressionTest):
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialVectorDoubleSubtractHost."
        msgThrownException = "Found exceptions in TestSerialVectorDoubleSubtractHost."
        msgAssertFail = "Found assert fail(s) in TestSerialVectorDoubleSubtractHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialVectorDoubleSubtractHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for subtracting two SerialVectors with double datatype'''
    target_name = 'TestSerialVectorDoubleSubtractHost'
    build_system = 'CMake'
