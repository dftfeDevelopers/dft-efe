...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleInPlaceSubtractDevice(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleInPlaceSubtractDevice using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleInPlaceSubtractDevice']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleInPlaceSubtractDevice(rfm.RegressionTest):
    descr = '''Compile and run test for in-place addition'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleInPlaceSubtractDevice']
    executable = './TestSerialDenseMatrixDoubleInPlaceSubtractDevice.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'gpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleInPlaceSubtractDevice."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleInPlaceSubtractDevice."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleInPlaceSubtractDevice."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleInPlaceSubtractDevice(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for testing in-place addition'''
    target_name = 'TestSerialDenseMatrixDoubleInPlaceSubtractDevice'
    build_system = 'CMake'
