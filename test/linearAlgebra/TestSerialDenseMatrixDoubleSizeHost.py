...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleSizeHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleSizeHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleSizeHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleSizeHost(rfm.RegressionTest):
    descr = '''Compile and run test for testing the function to get the matrix dimension of SerialDenseMatrix with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleSizeHost']
    executable = './TestSerialDenseMatrixDoubleSizeHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleSizeHost."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleSizeHost."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleSizeHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleSizeHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for testing the function to get the matrix dimension of SerialDenseMatrix with double datatype'''
    target_name = 'TestSerialDenseMatrixDoubleSizeHost'
    build_system = 'CMake'
