...
@rfm.simple_test
class BuildOnlyTestSerialVectorDoubleAddDevice(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialVectorDoubleAddDevice using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialVectorDoubleAddDevice']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialVectorDoubleAddDevice(rfm.RegressionTest):
    descr = '''Compile and run test for adding two SerialVectors with double datatype on Device'''
    build_system = 'CMake'
    make_opts = ['TestSerialVectorDoubleAddDevice']
    executable = './TestSerialVectorDoubleAddDevice.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'gpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialVectorDoubleAddDevice."
        msgThrownException = "Found exceptions in TestSerialVectorDoubleAddDevice."
        msgAssertFail = "Found assert fail(s) in TestSerialVectorDoubleAddDevice."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialVectorDoubleAddDevice(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for adding two SerialVectors with double datatype on Device'''
    target_name = 'TestSerialVectorDoubleAddDevice'
    build_system = 'CMake'
