...
@rfm.simple_test
class BuildOnlyTestRayleighRitzHostDouble(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestRayleighRitzHostDouble using CMake'
    build_system = 'CMake'
    make_opts = ['TestRayleighRitzHostDouble']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestRayleighRitzHostDouble(rfm.RegressionTest):
    descr = '''Compile and run test for computing norms of SerialMultiVector with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestRayleighRitzHostDouble']
    executable = './TestRayleighRitzHostDouble.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestRayleighRitzHostDouble."
        msgThrownException = "Found exceptions in TestRayleighRitzHostDouble."
        msgAssertFail = "Found assert fail(s) in TestRayleighRitzHostDouble."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestRayleighRitzHostDouble(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing norms of SerialMultiVector with double datatype'''
    target_name = 'TestRayleighRitzHostDouble'
    build_system = 'CMake'
