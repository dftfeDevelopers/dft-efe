...
@rfm.simple_test
class BuildOnlyTestChebyshevFilteredEigenSolveHostComplexDouble(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestChebyshevFilteredEigenSolveHostComplexDouble using CMake'
    build_system = 'CMake'
    make_opts = ['TestChebyshevFilteredEigenSolveHostComplexDouble']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestChebyshevFilteredEigenSolveHostComplexDouble(rfm.RegressionTest):
    descr = '''Compile and run test for computing norms of SerialMultiVector with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestChebyshevFilteredEigenSolveHostComplexDouble']
    executable = './TestChebyshevFilteredEigenSolveHostComplexDouble.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestChebyshevFilteredEigenSolveHostComplexDouble."
        msgThrownException = "Found exceptions in TestChebyshevFilteredEigenSolveHostComplexDouble."
        msgAssertFail = "Found assert fail(s) in TestChebyshevFilteredEigenSolveHostComplexDouble."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestChebyshevFilteredEigenSolveHostComplexDouble(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing norms of SerialMultiVector with double datatype'''
    target_name = 'TestChebyshevFilteredEigenSolveHostComplexDouble'
    build_system = 'CMake'
