...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleConstructorDevice(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleConstructorDevice using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleConstructorDevice']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleConstructorDevice(rfm.RegressionTest):
    descr = '''Compile and run test for constructors of SerialDenseMatrix with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleConstructorDevice']
    executable = './TestSerialDenseMatrixDoubleConstructorDevice.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'gpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleConstructorDevice."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleConstructorDevice."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleConstructorDevice."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleConstructorDevice(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for constructors of of SerialDenseMatrix with double datatype'''
    target_name = 'TestSerialDenseMatrixDoubleConstructorDevice'
    build_system = 'CMake'
