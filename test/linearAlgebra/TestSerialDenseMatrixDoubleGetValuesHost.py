...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleGetValuesHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleGetValuesHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleGetValuesHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleGetValuesHost(rfm.RegressionTest):
    descr = '''Compile and run test for testing the getValues and setStorage function of SerialDenseMatrix with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleGetValuesHost']
    executable = './TestSerialDenseMatrixDoubleGetValuesHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleGetValuesHost."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleGetValuesHost."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleGetValuesHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleGetValuesHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for testing getValues and setStorage of SerialDenseMatrix with double datatype'''
    target_name = 'TestSerialDenseMatrixDoubleGetValuesHost'
    build_system = 'CMake'
