...
@rfm.simple_test
class BuildOnlyTestSerialVectorDoubleInPlaceAddHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialVectorDoubleInPlaceAddHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialVectorDoubleInPlaceAddHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialVectorDoubleInPlaceAddHost(rfm.RegressionTest):
    descr = '''Compile and run test for inplace addition of SerialVectors with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialVectorDoubleInPlaceAddHost']
    executable = './TestSerialVectorDoubleInPlaceAddHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialVectorDoubleInPlaceAddHost."
        msgThrownException = "Found exceptions in TestSerialVectorDoubleInPlaceAddHost."
        msgAssertFail = "Found assert fail(s) in TestSerialVectorDoubleInPlaceAddHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialVectorDoubleInPlaceAddHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for inplace addition of SerialVectors with double datatype'''
    target_name = 'TestSerialVectorDoubleInPlaceAddHost'
    build_system = 'CMake'
