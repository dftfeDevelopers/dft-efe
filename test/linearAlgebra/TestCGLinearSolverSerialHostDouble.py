...
@rfm.simple_test
class BuildOnlyTestCGLinearSolverSerialHostDouble(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestCGLinearSolverSerialHostDouble using CMake'
    build_system = 'CMake'
    make_opts = ['TestCGLinearSolverSerialHostDouble']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestCGLinearSolverSerialHostDouble(rfm.RegressionTest):
    descr = '''Compile and run test for size of TestCGLinearSolverSerialHostDouble'''
    build_system = 'CMake'
    make_opts = ['TestCGLinearSolverSerialHostDouble']
    executable = './TestCGLinearSolverSerialHostDouble.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestCGLinearSolverSerialHostDouble."
        msgThrownException = "Found exceptions in TestCGLinearSolverSerialHostDouble."
        msgAssertFail = "Found assert fail(s) in TestCGLinearSolverSerialHostDouble."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestCGLinearSolverSerialHostDouble(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for size of SerialVector'''
    target_name = 'TestCGLinearSolverSerialHostDouble'
    build_system = 'CMake'
