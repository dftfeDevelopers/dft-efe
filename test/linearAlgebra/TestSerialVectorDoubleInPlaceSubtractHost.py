...
@rfm.simple_test
class BuildOnlyTestSerialVectorDoubleInPlaceSubtractHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialVectorDoubleInPlaceSubtractHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialVectorDoubleInPlaceSubtractHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialVectorDoubleInPlaceSubtractHost(rfm.RegressionTest):
    descr = '''Compile and run test for inplace subtraction of SerialVectors with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialVectorDoubleInPlaceSubtractHost']
    executable = './TestSerialVectorDoubleInPlaceSubtractHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialVectorDoubleInPlaceSubtractHost."
        msgThrownException = "Found exceptions in TestSerialVectorDoubleInPlaceSubtractHost."
        msgAssertFail = "Found assert fail(s) in TestSerialVectorDoubleInPlaceSubtractHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialVectorDoubleInPlaceSubtractHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for inplace subtraction of SerialVectors with double datatype'''
    target_name = 'TestSerialVectorDoubleInPlaceSubtractHost'
    build_system = 'CMake'
