...
@rfm.simple_test
class BuildOnlyTestSerialDenseMatrixDoubleInPlaceAddDevice(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialDenseMatrixDoubleInPlaceAddDevice using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleInPlaceAddDevice']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialDenseMatrixDoubleInPlaceAddDevice(rfm.RegressionTest):
    descr = '''Compile and run test for in-place addition'''
    build_system = 'CMake'
    make_opts = ['TestSerialDenseMatrixDoubleInPlaceAddDevice']
    executable = './TestSerialDenseMatrixDoubleInPlaceAddDevice.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'gpu',
                'serialOrParallel': 'serial'}
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialDenseMatrixDoubleInPlaceAddDevice."
        msgThrownException = "Found exceptions in TestSerialDenseMatrixDoubleInPlaceAddDevice."
        msgAssertFail = "Found assert fail(s) in TestSerialDenseMatrixDoubleInPlaceAddDevice."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialDenseMatrixDoubleInPlaceAddDevice(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for testing in-place addition'''
    target_name = 'TestSerialDenseMatrixDoubleInPlaceAddDevice'
    build_system = 'CMake'
