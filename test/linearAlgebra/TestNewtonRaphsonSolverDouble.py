...
@rfm.simple_test
class BuildOnlyTestNewtonRaphsonSolverDouble(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestNewtonRaphsonSolverDouble using CMake'
    build_system = 'CMake'
    make_opts = ['TestNewtonRaphsonSolverDouble']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestNewtonRaphsonSolverDouble(rfm.RegressionTest):
    descr = '''Compile and run test for computing norms of SerialMultiVector with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestNewtonRaphsonSolverDouble']
    executable = './TestNewtonRaphsonSolverDouble.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestNewtonRaphsonSolverDouble."
        msgThrownException = "Found exceptions in TestNewtonRaphsonSolverDouble."
        msgAssertFail = "Found assert fail(s) in TestNewtonRaphsonSolverDouble."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestNewtonRaphsonSolverDouble(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing norms of SerialMultiVector with double datatype'''
    target_name = 'TestNewtonRaphsonSolverDouble'
    build_system = 'CMake'
