...
@rfm.simple_test
class BuildOnlyTestParallelMultiVectorMultipassLowdinHostDouble(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestParallelMultiVectorMultipassLowdinHostDouble using CMake'
    build_system = 'CMake'
    make_opts = ['TestParallelMultiVectorMultipassLowdinHostDouble']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestParallelMultiVectorMultipassLowdinHostDouble(rfm.RegressionTest):
    descr = '''Compile and run test for computing norms of SerialMultiVector with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestParallelMultiVectorMultipassLowdinHostDouble']
    executable = './TestParallelMultiVectorMultipassLowdinHostDouble.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestParallelMultiVectorMultipassLowdinHostDouble."
        msgThrownException = "Found exceptions in TestParallelMultiVectorMultipassLowdinHostDouble."
        msgAssertFail = "Found assert fail(s) in TestParallelMultiVectorMultipassLowdinHostDouble."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestParallelMultiVectorMultipassLowdinHostDouble(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for computing norms of SerialMultiVector with double datatype'''
    target_name = 'TestParallelMultiVectorMultipassLowdinHostDouble'
    build_system = 'CMake'
