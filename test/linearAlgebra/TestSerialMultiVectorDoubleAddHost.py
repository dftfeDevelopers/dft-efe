...
@rfm.simple_test
class BuildOnlyTestSerialMultiVectorDoubleAddHost(rfm.CompileOnlyRegressionTest):
    descr = 'Compile only test for TestSerialMultiVectorDoubleAddHost using CMake'
    build_system = 'CMake'
    make_opts = ['TestSerialMultiVectorDoubleAddHost']
    sourcesdir = './src'
...
@rfm.simple_test
class BuildAndRunTestSerialMultiVectorDoubleAddHost(rfm.RegressionTest):
    descr = '''Compile and run test for adding two SerialMultiVectors with double datatype'''
    build_system = 'CMake'
    make_opts = ['TestSerialMultiVectorDoubleAddHost']
    executable = './TestSerialMultiVectorDoubleAddHost.x'
    sourcesdir = './src'
    tagsDict = {'compileOrRun': 'compile', 'unitOrAggregate':
                'unit', 'slowOrFast': 'fast', 'arch': 'cpu',
    ...
    def validate_test(self):
        ...
        hasError = True
        msgError = "Found error(s) in TestSerialMultiVectorDoubleAddHost."
        msgThrownException = "Found exceptions in TestSerialMultiVectorDoubleAddHost."
        msgAssertFail = "Found assert fail(s) in TestSerialMultiVectorDoubleAddHost."
        matchesOut = evaluate(sn.findall(r'(?i)error', evaluate(self.stdout)))
        matchesErr = evaluate(sn.findall(r'(?i)error', evaluate(self.stderr)))
        if len(matchesOut) == 0 and len(matchesErr) == 0:
            hasError = False
...
@rfm.simple_test
class RunOnlyTestSerialMultiVectorDoubleAddHost(rfm.RunOnlyRegressionTest):
    descr = '''Run only test for adding two SerialMultiVectors with double datatype'''
    target_name = 'TestSerialMultiVectorDoubleAddHost'
    build_system = 'CMake'
