               ...
               const double zeroLocallyOwnedProbability)
{
    ...
    for(size_type i = 0; i < nprocs; ++i)
    {
        ...
        size_type nLocallyOwned = (prob < zeroLocallyOwnedProbability) ? 
            0 : std::rand() % maxOwnedIds;
        ...
        const size_type nprocs)
{
    ...
    //
    std::vector<std::pair<size_type, global_size_type>> procIdAndOwnedEndIdPairs(0);
