...
using MemoryStorageComplexDoubleDevice =
  dftefe::utils::MemoryStorage<std::complex<double>,
                               dftefe::utils::MemorySpace::DEVICE>;

global_size_type
getAGhostIndex(const global_size_type numGlobalIndices,
...
main()
{
  ...
  const size_type        numGhostIndices =
    (numProcs == 1) ? 0 : std::rand() % maxNumGhostIndices;
  ...
  for (size_type i = 0; i < numGhostIndices; ++i)
    {
      global_size_type ghostIndex =
        getAGhostIndex(numGlobalIndices, ownedIndexStart, ownedIndexEnd);
  ...
  std::pair<global_size_type, global_size_type> locallyOwnedRange(
    std::make_pair(ownedIndexStart, ownedIndexEnd));
  std::vector<global_size_type> ghostIndices(numGhostIndices);
  std::copy(ghostIndicesSet.begin(),
            ghostIndicesSet.end(),
            ghostIndices.begin());
      ...
      if (iProc != rank)
        {
          ...
          dftefe::utils::throwException(readfile.is_open(),
                                        "The file " + readfilename +
                                          " does not exist");
          std::vector<global_size_type> globalIndices(0);
          while (std::getline(readfile, line))
            {
              dftefe::utils::throwException(!line.empty(),
                                            "Empty or invalid line in file " +
                                              readfilename + " Line: " + line);
          ...
          for (size_type i = 0; i < globalIndices.size(); ++i)
            {
              ...
              if (globalIndex >= ownedIndexStart && globalIndex < ownedIndexEnd)
                procIdToOwnedLocalIndices[iProc].push_back(globalIndex -
                                                           ownedIndexStart);
            }
  ...
  const size_type ownedPlusGhostSize =
    mpiPatternP2PPtr->localOwnedSize() + mpiPatternP2PPtr->localGhostSize();
  ...
  for (size_type i = ownedSize; i < ownedPlusGhostSize; ++i)
    {
      ...
      std::string  msg =
        ...
        " mismatch of ghost value for double and block size=1 case"
        " Expected ghost value: " +
        std::to_string(expectedVal) +
        "\n"
        " Received ghost value: " +
        std::to_string(dVecStd1[i]);
      dftefe::utils::throwException(std::abs(dVecStd1[i] - expectedVal) <=
                                      1e-10,
                                    msg);
    }
  ...
  for(size_type i = 0; i < ownedSize; ++i)
    dVecStd2[i] =
  std::complex<double>(mpiPatternP2PPtr->localToGlobal(i),-mpiPatternP2PPtr->localToGlobal(i));
  ...
  dftefe::utils::MPICommunicatorP2P<std::complex<double>,dftefe::utils::MemorySpace::DEVICE>
  mpiCommunicatorP2P2(mpiPatternP2PPtr,1);
  ...
  for(size_type i = ownedSize; i < ownedPlusGhostSize; ++i)
  {
      const std::complex<double> expectedVal=
  std::complex<double>(mpiPatternP2PPtr->localToGlobal(i),-mpiPatternP2PPtr->localToGlobal(i));
      std::string msg = "In rank " + std::to_string(rank) + " mismatch of ghost
  value for std::complex<double> and block size=1 case" " Expected ghost real
  value: " + std::to_string(expectedVal.real()) + "\n" " Received ghost real
  value: " + std::to_string(dVecStd2[i].real()) + "\n" " Expected ghost imag
  value: " + std::to_string(expectedVal.imag()) + "\n" " Received ghost imag
  value: " + std::to_string(dVecStd2[i].imag());
      dftefe::utils::throwException(std::abs(dVecStd2[i].real()-expectedVal.real())
  <=1e-10
                                         &&
  ...
  const size_type
  ownedSizeMultivector=(mpiPatternP2PPtr->localOwnedSize())*blockSize; const
  size_type
  ownedPlusGhostSizeMultivector=(mpiPatternP2PPtr->localOwnedSize()+mpiPatternP2PPtr->localGhostSize())*blockSize;
  std::vector<double> dVecStd3(ownedPlusGhostSizeMultivector,0.0);
  for(size_type i = 0; i < ownedSizeMultivector; ++i)
    dVecStd3[i] =
  mpiPatternP2PPtr->localToGlobal(i/blockSize)*blockSize+i%blockSize;
  ...
  mpiCommunicatorP2P3(mpiPatternP2PPtr,blockSize);

  mpiCommunicatorP2P3.updateGhostValues(memStorage3);

  memStorage3.copyTo<dftefe::utils::MemorySpace::HOST>(dVecStd3.data());

  for(size_type i = ownedSizeMultivector; i < ownedPlusGhostSizeMultivector;
  ++i)
  {
      const double
  expectedVal=mpiPatternP2PPtr->localToGlobal(i/blockSize)*blockSize+i%blockSize;
      std::string msg = "In rank " + std::to_string(rank) + " mismatch of ghost
  value for double and block size=3 case" " Expected ghost value: " +
  std::to_string(expectedVal) + "\n" " Received ghost value: " +
  std::to_string(dVecStd3[i]);
      dftefe::utils::throwException(std::abs(dVecStd3[i]-expectedVal) <=1e-10,
  msg);
  }
  */
