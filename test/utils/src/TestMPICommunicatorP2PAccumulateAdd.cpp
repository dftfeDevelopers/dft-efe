...
int main()
{
  ...
  std::vector<double> dexpectedValuesStd1(ownedSize,0.0);
  ...
  {
      const double expectedVal=dexpectedValuesStd1[i];
      std::string msg = "In rank " + std::to_string(rank) + " mismatch of accumulated owned value for double and block size=1 case"
    " Expected owned value: " + std::to_string(expectedVal) + "\n"
    " Received owned value: " + std::to_string(dVecStd1[i]);
  ...
  std::vector<std::complex<double>> dexpectedValuesStd2(ownedSize,0.0);  
  for(size_type i = ownedSize; i <  ownedPlusGhostSize; ++i)
  ...
  for(size_type i = 0; i < ownedSize; ++i)
  {
      const std::complex<double> expectedVal= dexpectedValuesStd2[i];
      std::string msg = "In rank " + std::to_string(rank) + " mismatch of accumulated owned value for std::complex<double> and block size=1 case"
    " Expected owned real value: " + std::to_string(expectedVal.real()) + "\n"
    " Received owned real value: " + std::to_string(dVecStd2[i].real()) + "\n"
    " Expected owned imag value: " + std::to_string(expectedVal.imag()) + "\n"
    " Received owned imag value: " + std::to_string(dVecStd2[i].imag());    
  ...
  std::vector<double> dexpectedValuesStd3(ownedSizeMultivector,0.0);  
  ...
  {
      const double expectedVal=dexpectedValuesStd3[i];
      std::string msg = "In rank " + std::to_string(rank) + " mismatch of accumulated owned value for double and block size=3 case"
    " Expected owned value: " + std::to_string(expectedVal) + "\n"
    " Received owned value: " + std::to_string(dVecStd3[i]);
