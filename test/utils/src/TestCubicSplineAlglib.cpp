...
int main()
{
  ...
  alglib::spline1dinterpolant alglibCSpline;
  alglib::ae_int_t natural_bound_type = 0;
  alglib::spline1dbuildcubic(alglibX,
      alglibY,
      numRows,
      natural_bound_type,
      0.0,
  ...
  //std::ofstream out;
  //out.open("outAlglibCSplineTestPoints");
  //out << std::setprecision(16);
  start = std::chrono::high_resolution_clock::now();
  for(unsigned int i = 0; i < numTestPoints; ++i)
  {
    testYData[i] = alglib::spline1dcalc(alglibCSpline, testXData[i]);
  ...
  //std::ofstream out1;
  //out1.open("outAlglibCSplineDerivativeTestPoints");
  ...
  outTiming.open("outTimingAlglibCSpline");
