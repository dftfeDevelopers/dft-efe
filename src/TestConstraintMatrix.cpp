...
double
interpolatePolynomial(unsigned int feOrder,
                      double       x_coord,
                      double       y_coord,
                      double       z_coord,
                      double       xmin,
                      ...
                      double       zmin)
{
  // The function should go to 0 at the boundaries for it to be compatible with
...
main()
{
  ...
  std::shared_ptr<dftefe::basis::FEBasisManager> basisManager =
    std::make_shared<dftefe::basis::FEBasisManagerDealii<dim>>(
      triangulationBase, feDegree);
  ...
  std::vector<std::shared_ptr<
    dftefe::basis::FEConstraintsBase<double, dftefe::utils::MemorySpace::HOST>>>
    constraintsVec;
  constraintsVec.resize(
    1,
    std::make_shared<
      dftefe::basis::
        FEConstraintsDealii<double, dftefe::utils::MemorySpace::HOST, dim>>());
  ...
  std::vector<std::shared_ptr<
    dftefe::basis::Constraints<double, dftefe::utils::MemorySpace::HOST>>>
    constraintsBaseVec(constraintsVec.size(), nullptr);
  std::copy(constraintsVec.begin(),
            constraintsVec.end(),
            constraintsBaseVec.begin());

  std::map<
    std::string,
    std::shared_ptr<const dftefe::basis::
                      Constraints<double, dftefe::utils::MemorySpace::HOST>>>
    constraintsMap;
  ...
  std::vector<dftefe::quadrature::QuadratureRuleAttributes> quadAttr(
    1,
    dftefe::quadrature::QuadratureRuleAttributes(
      dftefe::quadrature::QuadratureFamily::GAUSS, true, num1DGaussSize));

  dftefe::basis::BasisStorageAttributesBoolMap BasisStorageAttributesBoolMapObj;
  BasisStorageAttributesBoolMapObj
    [dftefe::basis::BasisStorageAttributes::StoreValues] = true;
  BasisStorageAttributesBoolMapObj
    [dftefe::basis::BasisStorageAttributes::StoreGradient] = false;
  BasisStorageAttributesBoolMapObj
    [dftefe::basis::BasisStorageAttributes::StoreHessian] = false;
  BasisStorageAttributesBoolMapObj
    [dftefe::basis::BasisStorageAttributes::StoreOverlap] = false;
  BasisStorageAttributesBoolMapObj
    [dftefe::basis::BasisStorageAttributes::StoreGradNiGradNj] = false;
  BasisStorageAttributesBoolMapObj
    [dftefe::basis::BasisStorageAttributes::StoreJxW] = false;
  BasisStorageAttributesBoolMapObj
    [dftefe::basis::BasisStorageAttributes::StoreQuadRealPoints] = false;


  dftefe::basis::QuadAttrToBasisStorageAttrMap QuadAttrToBasisStorageAttrMapObj;
  QuadAttrToBasisStorageAttrMapObj[quadAttr[0]] =
    BasisStorageAttributesBoolMapObj;

  // Set up the FE Basis Data Storage
  std::shared_ptr<
    dftefe::basis::BasisDataStorage<double, dftefe::utils::MemorySpace::HOST>>
    feBasisData = std::make_shared<
      dftefe::basis::FEBasisDataStorageDealii<double,
                                              dftefe::utils::MemorySpace::HOST,
                                              dim>>(
      basisManager, quadAttr, QuadAttrToBasisStorageAttrMapObj);

  // Set up BasisHandler
  std::shared_ptr<
    dftefe::basis::BasisHandler<double, dftefe::utils::MemorySpace::HOST>>
    basisHandler = std::make_shared<
      dftefe::basis::
        FEBasisHandlerDealii<double, dftefe::utils::MemorySpace::HOST, dim>>(
      basisManager, constraintsMap);

  // Set up basis Operations
  dftefe::basis::
    FEBasisOperations<double, double, dftefe::utils::MemorySpace::HOST, dim>
      feBasisOp(feBasisData, 1000);
  ...
  for (dftefe::size_type iCell = 0; iCell < numLocallyOwnedCells; iCell++)
    {
      ...
      for (dftefe::size_type iNode = 0; iNode < cellGlobalNodeIds.size();
           iNode++)
        {
          dftefe::global_size_type globalId = cellGlobalNodeIds[iNode];
          if (!constraintsVec[0]->isConstrained(globalId))
            {
              dftefe::size_type localId =
                basisHandler->globalToLocalIndex(globalId, constraintName);
              basisHandler->getBasisCenters(localId, constraintName, nodeLoc);

              *(itField + localId) = interpolatePolynomial(
                feDegree, nodeLoc[0], nodeLoc[1], nodeLoc[2], xmin, ymin, zmin);
              std::cout << "id = " << nodeCount << " local = " << localId
                        << " inVal = " << *(itField + localId) << "\n";
            }
          else
            {
              dftefe::size_type localId =
                basisHandler->globalToLocalIndex(globalId, constraintName);
              basisHandler->getBasisCenters(localId, constraintName, nodeLoc);
              std::cout << "id = " << nodeCount << " x = " << nodeLoc[0]
                        << " y = " << nodeLoc[1] << " z = " << nodeLoc[2]
                        << std::endl;
            }
          nodeCount++;
        }
  ...
  /*
    ...
    basisHandler->globalToLocalIndex(globalId,constraintName) ;
             ...
             std::cout<<"id = "<<nodeCount<<" local = "<<localId<<" inConsVal =
    "<<*(itField + localId )<<"\n";
            }
            nodeCount++;
          }
  ...
  */
  ...
  std::shared_ptr<dftefe::quadrature::QuadratureRule> quadRule =
    std::make_shared<dftefe::quadrature::QuadratureRuleGauss>(dim,
                                                              num1DGaussSize);
  ...
  dftefe::quadrature::QuadratureRuleContainer quadRuleContainer(
    quadAttr[0], quadRule, triangulationBase, linearCellMappingDealii);

  dftefe::quadrature::
    QuadratureValuesContainer<double, dftefe::utils::MemorySpace::HOST>
      quadValuesContainer(quadRuleContainer, 1);
  ...
  for (auto it = quadValuesContainer.begin(); it != quadValuesContainer.end();
       it++)
    {
      ...
      double analyticValue =
        interpolatePolynomial(feDegree, xLoc, yLoc, zLoc, xmin, ymin, zmin);

      if (std::abs((*it) - analyticValue) > 1e-8)
        {
          std::cout << " id = " << count << " x = " << xLoc << " y  = " << yLoc
                    << " z = " << zLoc << " analVal = " << analyticValue
                    << " interValue = " << (*it) << "\n";
          testPass = false;
        }
      count++;
