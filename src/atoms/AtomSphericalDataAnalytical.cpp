  ...
  namespace atoms
  {
      ...
      const SphericalHarmonicFunctions &sphericalHarmonicFunc)
    {
      ...
      utils::throwException(
        (itQNum != fieldToQuantumNumbersVec.end() &&
         itFun != fieldToScalarSpatialFnRealVec.end()),
        "Could not find field name in the input maps in AtomSphericalDataAnalytical");

      utils::throwException(
        (itQNum->second.size() == itFun->second.size()),
      ...
      for (size_type iQNum = 0; iQNum < N; ++iQNum)
        {
          ...
          if (std::abs((*itFun->second[iQNum])(
                utils::Point({d_radialPointMax, 0., 0.}))) > 1e-10)
            {
              cutoff     = 1e6;
              smoothness = 1e10;
            }
          ...
          sphericalDataVec.push_back(
            std::make_shared<SphericalDataAnalytical>(itQNum->second[iQNum],
                                                      *itFun->second[iQNum],
                                                      cutoff,
                                                      smoothness,
                                                      sphericalHarmonicFunc));
    ...
    std::string
    AtomSphericalDataAnalytical::getMetadata(
    ...
    size_type
    AtomSphericalDataAnalytical::getQNumberID(
      const std::string       fieldName,
      const std::vector<int> &qNumbers) const
    {
