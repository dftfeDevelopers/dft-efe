...
namespace dftefe
{
  namespace atoms
  {
    ...
    public:
       ...
       * @brief Constructor
       *
       * @param[in] atomSymbolToAtomFileMap map from the atomic symbol to the
       * filename containing the atomic data
       * @param[in] atomFieldName name of the atomic field that needs to be
       * stored
       */
      AtomsFieldDataSphericalManager(
        const std::map<std::string, std::string> &atomSymbolToAtomFileMap,
       ...
       * Each element in the 2D vector contains a vector of size 3 containing
       * the \f$n,l,m\f$ quantum numbers (see top for definitions), in that
       * sequence.
       */
       ...
       * @param[in] atomSymbol symbol of the atomic species
       * @param[in] quantumNumbers vector containing the \f$n,l,m\f$ quantum
       * numbers
       *
    ...
    private:
      std::map<std::string, std::shared_ptr<AtomFieldDataSpherical>>
        d_atomSymbolToAtomFieldDataSphericalMap;
    };
