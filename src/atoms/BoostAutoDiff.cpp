...
namespace dftefe
{
    namespace atoms
    {
        namespace BoostAutoDiffInternal
        {
            ...
            {
                assert(std::abs(m) <= l);
            return sqrt(((2.0 * l + 1) * boost::math::factorial<double>(l - std::abs(m))) /
                            (2.0 * boost::math::factorial<double>(l + std::abs(m))));
            }

            template <typename X, typename Y, typename Z>
            boost::math::differentiation::promote<X,Y,Z> radVal(const X& x, const Y& y, const Z& z, 
                                                const std::vector<double> & coeffVec)
            {
                    auto r = sqrt(x*x + y*y + z*z);
                    auto radValue = coeffVec[1] + coeffVec[2]*(r-coeffVec[0]) + coeffVec[3]*pow((r-coeffVec[0]),2) + coeffVec[4]*pow((r-coeffVec[0]),3);
            return radValue;
            }

            template <typename X, typename Y, typename Z>
            boost::math::differentiation::promote<X,Y,Z> cutOff(const X& x, const Y& y, const Z& z, const double smoothness, 
                                                const double cutoff)
            {
                    auto r = sqrt(x*x + y*y + z*z);
                    auto smoothCutoff = (exp(-1.0/(1-smoothness*(r-cutoff)/cutoff))/
            ...
            boost::math::differentiation::promote<X,Y,Z> legendre(const X& x, const Y& y, const Z& z, const int l, const int m)
            {
                ...
                if ( m != l )
                {
                    ...
                    {
                        cx = ( ( 2 * i - 1 ) * z1 * cxPrev + ( - i - m + 1 ) *  cxPrevPrev ) / ( i - m );
                        cxPrevPrev = cxPrev;
                        cxPrev = cx;
                    }
                }
            ...
            template <typename X, typename Y, typename Z>
            boost::math::differentiation::promote<X,Y,Z> f(const X& x, const Y& y, const Z& z, 
                const std::vector<double> & coeffVec, const double smoothness, const double cutoff, const int l, 
                const int m, const double        polarAngleTolerance)
            {
                auto r = sqrt(x*x + y*y + z*z);
                ...
                if ( (double)r <= cutoff )
                {
                    ...
                    if (m < 0)
                    {
                        retValue = radValue * Clm(l, m) * Dm(m) * lege * sin(std::abs(m) * phi);
                    }
                    if (m == 0)
                    {
                ...
                else if ( (double)r > cutoff && (double)r <= cutoffmax)
                {
                    if (m > 0)
                    {
                        retValue = radValue * smoothCutoff * Clm(l, m) * Dm(m) * lege * cos(m * phi);
                    }
                    if (m < 0)
                    {
                        retValue = radValue * smoothCutoff * Clm(l, m) * Dm(m) * lege * sin(std::abs(m) * phi);
                    }
                    if (m == 0)
                    {
                    ...
                    const double polarAngleTolerance)
        {
            ...
            auto const variables = boost::math::differentiation::make_ftuple<double, Nx, Ny, Nz>(r1, r2, r3);
            auto const& x = std::get<0>(variables);
            auto const& y = std::get<1>(variables);
            auto const& z = std::get<2>(variables);
            auto const v = BoostAutoDiffInternal::f(x, y, z, coeffVec, smoothness, cutoff, l, m, polarAngleTolerance);
