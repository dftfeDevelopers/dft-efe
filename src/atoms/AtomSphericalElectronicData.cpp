  ...
  namespace atoms
  {
    ...
    namespace
    {
        ...
        const AtomSphericalElectronicDataXMLLocal::XPathInfo &xPathInfo)
      {
        ...
        AtomSphericalElectronicDataXMLLocal::readSphericalDataFromXMLNodeData(
          radialValuesStrings, qNumbersStrings, cutOffInfoStrings, xPathInfo);
        AtomSphericalElectronicDataXMLLocal::
    ...
    AtomSphericalElectronicData::AtomSphericalElectronicData(
      ...
      , d_Z(0.0)
    {
      ...
      AtomSphericalElectronicDataXMLLocal::getNodeStrings(xPathInfo,
                                                          nodeStrings);
      ...
      utils::throwException(numRadialPoints > 0,
                            "Negative integer found for" + xPathInfo.xpath +
                              " element in " + filename);
      ...
      convSuccess = AtomSphericalElectronicDataXMLLocal::splitStringToDoubles(
        nodeStrings[0], d_radialPoints, d_numRadialPoints);
      utils::throwException(convSuccess,
                            "Error while converting " + xPathInfo.xpath +
                              " element in " + filename + " to double");
      utils::throwException(
      ...
      utils::throwException(
        d_vHartreeData.size() == d_vNuclearData.size(),
        "Mismatch in number of total hartree and nuclear potential found in file: " +
          d_filename);

      // get total potential values (total potential = hartree potential +
      // nuclear potential)
      ...
      utils::throwException(
        d_vTotalData.size() == d_vNuclearData.size(),
