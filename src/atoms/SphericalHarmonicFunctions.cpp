    ...
    namespace atoms
    {
                ...
                double polarAngleTolerance)
        {
            ...
            else
            {
                ...
                //
                if(fabs(theta - 0.0) >= polarAngleTolerance && fabs(theta - M_PI) >= polarAngleTolerance)
                    phi = atan2(x[1],x[0]);
                else
                    phi = 0.0;
            }
        ...
        //
        // We use the real form of spherical harmonics without the Condon-Shortley phase (i.e., the (-1)^m prefactor)
        // (see https://en.wikipedia.org/wiki/Spherical_harmonics) 
        // NOTE: 1) The wikipedia definition has the Condon-Shortley phase. 
        //       2) The definition of the associated Legendre polynomial (P_lm) in Boost library also contains a Condon-Shortley phase.
        //          Thus, if you're using Boost library, multiply the P_lm evaluation with (-1)^m to remove the Condon-Shortley phase.
        //          Most Quantum Chemistry codes (e.g., QChem) do not include the Condon-Shortley phase. So to make it consistent, it
        //          is prefered to remove the Condon-Shortley phase, if there are any to begin with.
        //        3) From C++17 onwards, the <cmath> has the associated Legendre polynomial (see https://en.cppreference.com/w/cpp/numeric/special_functions/assoc_legendre) 
        //           Thus, if you're using C++17 or beyond, you can use the C++ standard's definition of associated Legendre polynomial instead of Boost.
        //           Note that, the C++ standard does not have the Condon-Shortley phase while Boost has it. So, we do not have to do anything special to remove it while using the C++ standard.
        //
        ...
        //
        // For the definition of the associated Legendre polynomials i.e. P_lm and their derivatives 
        ...
        //        publisher={Elsevier}
        //       }
        // We use the derivative definitions from the above reference because finding the derivatives on the pole (i.e., theta = 0) is tricky. 
        // This is because the azimuthal angles (phi) is undefined for a point on the pole. However, the derivative is still well defined on the pole via the L'Hospital's rule.
        // However, one can avoid implementing tedious L'Hospital's rule on pole and use much simpler expressions given in the above reference.
        //
        ...
        double Clm(const int l, const int m)
        {
            ...
            //assert(m >= 0);
            assert(std::abs(m) <= l);
            return sqrt(((2.0*l + 1)*boost::math::factorial<double>(l-abs(m)))/(2.0*boost::math::factorial<double>(l+abs(m))));
        }
