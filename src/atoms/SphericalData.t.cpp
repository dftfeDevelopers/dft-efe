...
namespace dftefe
{
  namespace atoms
  {
    ...
    template <size_type dim>
    std::vector<double>
    SphericalData::getDerivativeValue(const utils::Point &point, 
                            const utils::Point &origin,
                            ...
                            const double cutoffTolerance) const
    {
      ...
      retValue[0] = dValueDR * (sin(theta)*cos(phi)) + dValueDThetaByr * (cos(theta)*cos(phi))
                    - sin(phi) * dValueDPhiByrsinTheta;
      retValue[1] = dValueDR * (sin(theta)*sin(phi)) + dValueDThetaByr * (cos(theta)*sin(phi))
