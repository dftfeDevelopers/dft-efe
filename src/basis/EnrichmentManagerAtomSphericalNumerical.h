...
namespace dftefe
{
  namespace basis
  {
     ...
     * This specifically assumes the <b> dimensionality of the problem to be 3</b>.
     * Thus, an enrichment function \f$ N^{\boldsymbol{\textbf{R}}}(\boldsymbol{\textbf{r}})\f$,
     * centered on a point \f$\boldsymbol{\textbf{R}}\f$ can be written as
     * \f{equation*}{
     *  N^{\boldsymbol{\textbf{r}}}(\boldsymbol{\textbf{r}}) = f(r) Y_{lm}(\theta,\phi)
     * \f}
     * where \f$r\f$ is the distance of \f$\boldsymbol{\textbf{r}}\f$ from \f$\boldsymbol{\textbf{R}}\f$;
     * \f$f(r)\f$ is the numerical radial part which is provided on a 1D grid.
     * \f$Y_{lm}(\theta,\phi)\f$ is a spherical harmonic of degree \f$l\f$ and order \f$m\f$, 
     * and \f$\theta\f$  and \f$\phi\f$ are the azimuthal and polar angles for the point \f$\boldsymbol{\textbf{r}}\f$ with the origin at \f$\boldsymbol{\textbf{R}}\f$.
	...
	public:
	   ...
	   * @brief Constructor
	   * 
	   * @param[in] atomSymbols vector containing the symbols of all the atoms in the system
	   * @param[in] atomCoordinates vector containing the coordinates of all the atoms in the system
	   * @param[in] atomsData AtomsData object containing the radial and angular data for the atomic species 
	   * in the system
	   */
	  EnrichmentManagerAtomSphericalNumerical(const std::vector<std::string>> & atomSymbols,
	  ...
	  virtual 
	    ValueType
	    getValue(const size_type functionId, const std::vector<double> & point) const = 0;
	   ...
	   *
	   * @param[in] functionId index of the enrichment function
	   * @param[in] points vector of Point containing the coordinates of all the input points
	   * 
	   ...
	   *
	   * @param[in] functionId index of the enrichment function
	   * @param[in] points 2D vector containing the coordinates of all the input points.
	   * The expected format is such that points[i][j] should correspond to the j-th coordinate
	   * of the i-th point.
	   * 
	   * @return values of the enrichment function on all the input points
	   */ 
	   ...
	   * The i-th value in the value denotes the derivative with respect to the i-th coordinate of the point. 
	   */ 
	  virtual 
	    std::vector<ValueType>
	    getDerivativeValue(const size_type functionId, const utils::Point & point, const size_type derivativeOrder) const = 0;
	   ...
	   * 
	   * @return vector containing the derivative of the enrichment function at the point.
	   * The i-th value in the vector denotes the derivative with respect to the i-th coordinate of the point. 
	   */ 
	  virtual 
	    ValueType
	    getDerivativeValue(const size_type functionId, const std::vector<double> & point, const size_type derivativeOrder) const = 0;
	   ...
	   * The (i,j)-th value in the 2D vector denotes the derivative with respect to the j-th coordinate of the i-th point. 
	   */ 
	  virtual 
