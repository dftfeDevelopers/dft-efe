...
namespace dftefe
{
  namespace basis
  {
    ...
    template <size_type dim>
    double FEBasisManagerDealii<dim>::
    getBasisFunctionValue(const size_type     basisId,
                          const utils::Point &point) const
    {
      utils::throwException(
        false, "getBasisFunctionValue() in FEBasisManagerDealii not yet implemented.");

    }

    template <size_type dim>
    std::vector<double> FEBasisManagerDealii<dim>::
    ...
    {
      utils::throwException(
        false, "getBasisFunctionDerivative() in FEBasisManagerDealii not yet implemented.");

    }

    template <size_type dim>
    void FEBasisManagerDealii<dim>::
    ...
    template <size_type dim>
    size_type FEBasisManagerDealii<dim>::
    nLocallyActiveCells() const
    {
      utils::throwException(
        false, "nLocallyActiveCells() in FEBasisManagerDealii not yet implemented.");
    }
    ...
    {
      utils::throwException(
        false, "nLocallyOwnedCells() in FEBasisManagerDealii not yet implemented.");
    ...
    template <size_type dim>
    global_size_type FEBasisManagerDealii<dim>::
    nGlobalNodes() const
    {
    ...
    template <size_type dim>
    std::vector<size_type> FEBasisManagerDealii<dim>::
    ...
    template <size_type dim>
    FEBasisManager<dim>::std::vector<std::shared_ptr<FECellBase>>::const_iterator
    FEBasisManagerDealii<dim>::
    ...
    template <size_type dim>
    unsigned int FEBasisManagerDealii<dim>::
    getDim() const
    {
      return dim;
    }
