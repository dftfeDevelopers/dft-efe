    ...
    namespace FEBasisDataStorageDealiiInternal
    {
        ...
        const bool                      storeHessians)
      {
        ...
        if (quadratureFamily == quadrature::QuadratureFamily::GAUSS)
        ...
        else
          {
            utils::throwException(
              false,
              "Storing of basis values for classical finite element basis is only provided for Gauss and Gauss-Legendre-Lobatto quadrature rule");
          }
        ...
        utils::throwException(
          ...
          "Storing of basis values for classical finite element basis on non-tensor structured quadrature grid is not supported");
        const size_type num1DQuadPoints =
          quadratureRuleAttributes.getNum1DPoints();
        const size_type numQuadPoints =
          utils::mathFunctions::sizeTypePow(num1DQuadPoints, dim);
        const size_type nDimxDofsPerCellxNumQuad =
          dim * dofsPerCell * numQuadPoints;
        const size_type nDimSqxDofsPerCellxNumQuad =
          dim * dim * dofsPerCell * numQuadPoints;
        ...
        if (storeValues)
          {
            basisQuadStorage = std::make_shared<
              typename BasisDataStorage<ValueType, memorySpace>::Storage>(
              dofsPerCell * numQuadPoints);
            basisQuadStorageTmp.resize(dofsPerCell * numQuadPoints,
                                       ValueType(0));
          }
	    ...
	    // quadraure rule in all elements, the classical FE basis values remain the same across
	    // as in the reference cell (unit n-dimensional cell). Thus, to optimize on memory
	    // we only store the classical FE basis values on the first cell
	    //
            if (storeValues &&
                ...
                for (unsigned int iNode = 0; iNode < dofsPerCell; iNode++)
                  {
                    for (unsigned int qPoint = 0; qPoint < nQuadPointInCell;
                         qPoint++)
                      {
                        *basisQuadStorageTmpIter = dealiiFEObj.shape_value(
                          iNode, dealiiParametricQuadPoints[qPoint]);
                        basisQuadStorageTmpIter++;
                      }
                  }
              }
                ...
                for (unsigned int jNode = 0; jNode < dofsPerCell; jNode++)
                  {
                    *basisOverlapTmpIter = 0.0;
                    for (unsigned int qPoint = 0; qPoint < numQuadPoints;
                      ...
                      {
                        *basisOverlapTmpIter +=
                          dealiiFEObj.shape_value(
                            iNode, dealiiParametricQuadPoints[qPoint]) *
                          dealiiFEValues.shape_value(
                            jNode, dealiiParametricQuadPoints[qPoint]) *
