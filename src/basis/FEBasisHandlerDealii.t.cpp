...
namespace dftefe 
{
  ...
  namespace basis
  {
	...
	size_type
	getLocallyOwnedCellsCumulativeDofs(const FEBasisManagerDealii<dim> * feBMDealii)
	{
      ...
      template<size_type dim>
	void
	getLocallyOwnedCellStartIds(const FEBasisManagerDealii<dim> * feBMDealii,
	    std::vector<size_type> & locallyOwnedCellStartIds)
	{
	  ...
	  for(size_type iCell = 0; iCell < numLocallyOwnedCells; ++iCell)
	  {
	    ...
	    {
	      locallyOwnedCellStartIds[iCell] = locallyOwnedCellStartIds[iCell-1] + 
      ...
      template<size_type dim>
	void
	getLocallyOwnedCellGlobalIndices<dim>(const FEBasisManagerDealii<dim> * feBMDealii,
	    std::vector<global_size_type> & locallyOwnedCellGlobalIndices)
	{
	  ...
	  for(size_type iCell = 0; iCell < numLocallyOwnedCells; ++iCell)
	  {
		...
		locallyOwnedCellGlobalIndices.begin()+cumulativeCellDofs);
	...
	void
      setDealiiMatrixFreeLight(const FEBasisManagerDealii<dim> * feBMDealii,
	  const std::map<std::string, std::shared_ptr<const FEBasisConstraintsDealii<dim, ValueType>>> & feConstraintsDealiiMap,
	...
	std::shared_ptr<const dealii::DoFHandler<dim>> dofHandler = feBMDealii->getDoFHandler();
	...
	for(auto it = feConstraintsDealiiMap.begin(); it != d_feConstraintsDealiiMap.end(); ++it)
	{
	...
	dealiiMatrixFree.reinit(dofHandlerVec,
      ...
      template<typename ValueType, size_type dim>
	void
	getGhostIndices(const dealii::MatrixFree<dim, ValueType>> & dealiiMatrixFree,
    ...
    template <typename ValueType, dftefe::utils::MemorySpace memorySpace, size_type dim>
      getLocallyOwnedCellLocalIndices(const FEBasisManagerDealii<dim> * feBMDealii,
	...
	for(size_type iCell = 0; iCell < numLocallyOwnedCells; ++iCell)
	{
	  ...
	  {
	    const global_size_type globalId = locallyOwnedCellGlobalIndices[cumulativeDofs+iDof];
	    locallyOwnedCellLocalIndices[cumulativeDofs+iDof] = mpiPatternP2P->globalToLocal(globalId);
	  }
	...
	const size_type cumulativeCellDofs = 
	  FEBasisHandlerDealiiInternal::getLocallyOwnedCellsCumulativeDofs<dim>(d_feBMDealii.get());

	//
	// populate d_locallyOwnedCellStartIds
	//
	std::vector<size_type> locallyOwnedCellStartIdsTmp(numLocallyOwnedCells,0);
	FEBasisHandlerDealiiInternal::getLocallyOwnedCellStartIds<dim>(d_feBMDealii.get(),
	    locallyOwnedCellStartIdsTmp);
	...
	FEBasisHandlerDealiiInternal::getLocallyOwnedCellGlobalIndices<dim>(d_feBMDealii.get(),
	...
	//
	dealii::MatrixFree<dim, ValueType>> dealiiMatrixFree;
	FEBasisHandlerDealiiInternal::setDealiiMatrixFreeLight<ValueType, dim>(d_feBMDealii.get(),
	...
	for(auto it = d_feConstraintsDealiiMap.begin(); it != d_feConstraintsDealiiMap.end(); ++it)
	{
	  ...
	  FEBasisHandlerDealiiInternal::getGhostIndices<ValueType,dim>(dealiiMatrixFree, iConstraint, ghostIndicesTmp);
	  ...
	  FEBasisHandlerDealiiInternal::getLocallyOwnedCellLocalIndices(d_feBMDealii.get(),
	      mpiPatternP2P.get(),
	      locallyOwnedCellGlobalIndicesTmp,
	      locallyOwnedCellLocalIndicesTmp);
	  auto globalSizeVector = std::make_shared<typename BasisHandler<memorySpace>::GlobalSizeTypeVector>(cumulativeDofs,0);
	  utils::MemoryTransfer<memorySpace, utils::MemorySpace::HOST>::copy(
	      cumulativeDofs,
	      globalSizeVector->data(),
