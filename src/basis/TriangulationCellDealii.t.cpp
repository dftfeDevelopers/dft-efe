...
namespace dftefe
{
  namespace basis
  {
   ...
   template <unsigned int dim>
   double TriangulationCellDealii<dim>::diameter() const
   {
     return d_cellItr->diameter(); 
   }

   template <unsigned int dim>
   void TriangulationCellDealii<dim>::center(dftefe::utils::Point &centerPoint) const
   {
    dealii::Point<dim,double> dealiiPoint;
   ...
   template <unsigned int dim>
   void TriangulationCellDealii<dim>::setRefineFlag()
   {
