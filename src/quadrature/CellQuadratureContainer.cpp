...
namespace dftefe {
  ...
  namespace quadrature {
	...
	//d_quadratureRuleVec.resize(numCells, std::shared_ptr<QuadratureRule> (new QuadratureRule()));
  ...
  unsigned int numQuadPoints = 0;
	...
	d_realPoints.resize(numQuadPoints, dftefe::utils::Point(triangulation->getDim(),0.0));
	for(unsigned int iCell = 0; iCell < numCells; ++iCell)
	{
