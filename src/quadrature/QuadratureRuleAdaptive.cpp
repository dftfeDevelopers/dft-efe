  ...
  namespace quadrature
  {
    namespace
    {
        ...
        std::vector<double> &               adaptiveQuadWeights)
      {
        ...
        for (dftefe::size_type iPoint = 0; iPoint < numberBaseQuadPoints; ++iPoint)
          {
             ...
             ++iFunction)
          {
            ...
            if (fabs(parentIntegral) >
                                            ...
                                            INTEGRAL_THRESHOLDS_NORMALIZATION))
              {
                ...
                for (dftefe::size_type iChild = 0; iChild < numberChildren; ++iChild)
                  {
                    sumChildIntegrals +=
        ...
        else
          {
            ...
            if (convergenceFlag)
            ...
            else
              {
                for (dftefe::size_type iChild = 0; iChild < numberChildren; ++iChild)
                  {
                    const dftefe::size_type recursionLevelNext = recursionLevel + 1;
      ...
      const dftefe::size_type             maxRecursion /*= 100*/)
    {
      ...
      for (dftefe::size_type iFunction = 0; iFunction < numberFunctions; ++iFunction)
        {
