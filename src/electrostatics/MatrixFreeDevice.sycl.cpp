...
inline dftefe::uInt
getMultiVectorIndex(const dftefe::uInt  node,
                            ...
                            sycl::local_accessor<T, 1> sharedConstrainingData)
{
  ...
  const dftefe::uInt blockIdxX  = item.get_group(2);
  const dftefe::uInt blockIdxY  = item.get_group(1);
  const dftefe::uInt threadIdxX = item.get_local_id(2);
  const dftefe::uInt threadIdxY = item.get_local_id(1);

  dftefe::uInt constrainingBucketStart = constrainingNodeOffset[blockIdxX];
  dftefe::uInt constrainingBucketSize =
    constrainingNodeOffset[blockIdxX + 1] - constrainingNodeOffset[blockIdxX];

  for (dftefe::uInt k = threadIdxY; k < constrainingBucketSize; k += yThreads)
    {
      dftefe::uInt idx;
  ...
  dftefe::uInt constrainedBucketStart = constrainedNodeOffset[blockIdxX];
  dftefe::uInt constrainedBucketSize =
    constrainedNodeOffset[blockIdxX + 1] - constrainedNodeOffset[blockIdxX];
  dftefe::uInt weightMatrixStart = weightMatrixOffset[blockIdxX];

  T inhomogenity = inhomogenityList[blockIdxX];

  for (dftefe::uInt j = threadIdxY; j < constrainedBucketSize; j += yThreads)
    {
      T tmp = inhomogenity;

      for (dftefe::uInt k = 0; k < constrainingBucketSize; k++)
        tmp +=
  ...
  sycl::local_accessor<T, 1> sharedConstrainedData)
{
  ...
  if (constrainingBucketSize > 0)
    {
      for (dftefe::uInt k = threadIdxY; k < constrainedBucketSize; k += yThreads)
        {
          dftefe::uInt idx;
      ...
      for (dftefe::uInt j = threadIdxY; j < constrainingBucketSize;
           j += yThreads)
        {
          T tmp = T(0);

          for (dftefe::uInt k = 0; k < constrainedBucketSize; k++)
              ...
              sycl::local_accessor<T, 1> sharedMem)
{
  ...
  const dftefe::uInt blockIdxX  = item.get_group(2);
  const dftefe::uInt blockIdxY  = item.get_group(1);
  const dftefe::uInt gridDimX   = item.get_group_range(2);
  const dftefe::uInt threadIdxX = item.get_local_id(2);
  const dftefe::uInt threadIdxY = item.get_local_id(1);
  ...
  const dftefe::uInt mapOffset = (blockIdxX + blockIdxY * gridDimX) *
  ...
  for (std::uint32_t i = threadIdxY; i < nDofsPerDim * nDofsPerDim;
       i += yThreads)
    {
      ...
      for (std::uint32_t k = 0; k < nDofsPerDim; k++)
        {
          dftefe::uInt dof = map[i + k * nDofsPerDim * nDofsPerDim + mapOffset];
  ...
  for (std::uint32_t i = threadIdxY; i < nQuadPointsPerDim * nQuadPointsPerDim;
       i += yThreads)
    {
      ...
      dftefe::uInt jOffset = blockIdxX * dim * dim;
  ...
  for (std::uint32_t i = threadIdxY; i < nDofsPerDim * nDofsPerDim;
       i += yThreads)
    {
      ...
      for (std::uint32_t j = 0; j < pOdd; j++)
        {
          dftefe::uInt dof1 = map[j + i * nDofsPerDim + mapOffset];
      ...
      if constexpr (nDofsPerDim % 2 == 1)
        {
          dftefe::uInt dof = map[pOdd + i * nDofsPerDim + mapOffset];
