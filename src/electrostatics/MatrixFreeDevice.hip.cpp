  ...
  const dftefe::uInt nGhostDofs)
{
  ...
  dftefe::uInt weightMatrixStart = weightMatrixOffset[blockIdx.x];

  T inhomogenity = inhomogenityList[blockIdx.x];

  for (dftefe::uInt j = threadIdx.y; j < constrainedBucketSize; j += yThreads)
    {
      T tmp = inhomogenity;
  ...
  const dftefe::uInt nGhostDofs)
{
  ...
  if (constrainingBucketSize > 0)
    {
      for (dftefe::uInt k = threadIdx.y; k < constrainedBucketSize;
           k += yThreads)
        {
          dftefe::uInt idx;
      ...
      for (dftefe::uInt j = threadIdx.y; j < constrainingBucketSize;
           j += yThreads)
        {
          T tmp = T(0);
...
#pragma unroll
  ...
  for (std::uint32_t i = threadIdx.y; i < nQuadPointsPerDim * nQuadPointsPerDim;
       i += yThreads)
    {
      ...
      dftefe::uInt jOffset = blockIdx.x * dim * dim;
...
#pragma unroll
      for (std::uint32_t j = 0; j < pOdd; j++)
        {
          dftefe::uInt dof1 = __ldg(&map[j + i * nDofsPerDim + mapOffset]);
      ...
      if constexpr (nDofsPerDim % 2 == 1)
        {
          dftefe::uInt dof = __ldg(&map[pOdd + i * nDofsPerDim + mapOffset]);
