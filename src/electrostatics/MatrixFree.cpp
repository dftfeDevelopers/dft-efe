...
#include <electrostatics/MatrixFree.h>
...
namespace dftefe
{
  ...
  {
    AssertThrow(memorySpace == dftefe::utils::MemorySpace::DEVICE,
                dealii::ExcMessage(
             ...
             subBatchSize>::init()
  {
    ...
    std::map<dealii::CellId, dftefe::uInt> cellIdToCellIndexMap;
    std::vector<dftefe::uInt> cellIndexToMacroCellSubCellIndexMap(d_nCells);
    ...
    iCell = 0;
    for (dftefe::uInt iMacroCell = 0; iMacroCell < d_nMacroCells; iMacroCell++)
      {
        const dftefe::uInt numberSubCells =
          d_matrixFreeDataPtr->n_active_entries_per_cell_batch(iMacroCell);

        for (dftefe::uInt iSubCell = 0; iSubCell < numberSubCells; iSubCell++)
          {
            cellPtr = d_matrixFreeDataPtr->get_cell_iterator(iMacroCell,
                                                             iSubCell,
                                                             d_dofHandlerID);

            dftefe::uInt cellIndex = cellIdToCellIndexMap[cellPtr->id()];
    ...
    constexpr dftefe::uInt dim = 3;
    dftefe::utils::MemoryStorage<T, dftefe::utils::MemorySpace::HOST>
      jacobianFactorTemp(dim * dim * d_nCells),
      jacobianFactor(dim * dim * d_nCells);

    auto cellOffsets = mappingData.data_index_offsets;

    for (dftefe::uInt iCellBatch = 0, cellCount = 0;
         iCellBatch < dofInfo.n_vectorization_lanes_filled[2].size();
         iCellBatch++)
      for (dftefe::uInt iCell = 0;
           iCell < dofInfo.n_vectorization_lanes_filled[2][iCellBatch];
           iCell++, cellCount++)
        for (dftefe::uInt k = 0; k < dim; k++)
          for (dftefe::uInt j = 0; j < dim; j++)
            for (dftefe::uInt i = 0; i < dim; i++)
    ...
    for (dftefe::uInt iCell = 0; iCell < d_nCells; iCell++)
      for (dftefe::uInt iDim = 0; iDim < dim * dim; iDim++)
    ...
    dftefe::utils::MemoryStorage<dftefe::uInt, dftefe::utils::MemorySpace::HOST>
    ...
    for (dftefe::uInt iCell = 0; iCell < d_nCells; iCell++)
      {
        ...
        std::transform(checkExpr ? trueClause : falseClause,
                       ...
                       [](unsigned int &v) {
                         return static_cast<dftefe::uInt>(v);
                       });
      }

    // Reorder cell numbering to cell-matrix order
    for (dftefe::uInt iCell = 0; iCell < d_nCells; iCell++)
      for (dftefe::uInt iDof = 0; iDof < d_nDofsPerCell; iDof++)
      ...
      if (d_constraintMatrixPtr->is_constrained(*iter))
        {
          ...
          for (dftefe::uInt j = 0; j < rowData->size(); j++)
            {
          ...
          if (constraintExists)
            ...
            {
              d_constrainedNodeBuckets.push_back(std::vector<dftefe::uInt>(
                1,
                d_matrixFreeDataPtr->get_vector_partitioner(d_dofHandlerID)
                  ->global_to_local(lineDof)));
