...
namespace dftefe
{
  namespace utils
  {
    ...
    private:
      ...
      /// Vector of locally owned indices which other processors need to
      /// imported Non-decreasing, but can have repeated entries
      std::vector<global_size_tye> d_importIndices;

      /// Vector of size 2*(number of ghost procs), where a ghost proc is one
      /// that owns at least one of the ghost indices of this processor. The
      /// entries as arranged as: <ghost proc1 ID> <number of ghost indices
      /// owned> <ghost proc2 ID> <number of ghost indices owned> ...
      std::vector<size_type> d_ghostProcIdToNumGhostMap;

      /// Vector of size 2*(number of import procs), where an import proc is one
      /// which imports at least one of the locally owned indices of this
      /// processor. The entries are arranged as: <import proc1 ID> <number of
      /// import indices> <import proc2 ID> <number of import indices> ...
