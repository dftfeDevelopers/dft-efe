...
namespace dftefe {
  namespace utils {
	...
	double getFirstDerivativeValue(const double & x) const;
	std::vector<double>
	  getFirstDerivativeValue(const std::vector<double> & x) const ;

	double getSecondDerivativeValue(const double & x) const;
	std::vector<double>
	  getSecondDerivativeValue(const std::vector<double> & x) const;

      private:
	static std::map<BoundaryType, alglib::ae_int_t> d_boundaryTypeToAlglibIntegerMap;
