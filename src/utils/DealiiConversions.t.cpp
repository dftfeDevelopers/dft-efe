...
namespace utils
{
  ...
  template <unsigned int dim>
  void
  convertToDealiiPoint(const std::vector<double> &v, dealii::Point<dim, double> & outputDealiiPoint)
  {
    utils::Assert(
      dim == v.size(),
...
template <unsigned int dim>
void
convertToDftefePoint(const dealii::Point<dim, double> & dealiiPoint, Point &outputDftefePoint)
{
