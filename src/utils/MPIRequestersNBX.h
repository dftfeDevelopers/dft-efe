...
namespace dftefe
{
  namespace utils
  {
    class MPIRequestersNBX
    {
      ...
      /*
       * The following is a brief description of the typical use case situation.
       * Each processor has a list of target processors to which it wants to
       * send a message (think of it as a message to another processor to
       * request some data that is owned by the other processor).
       ...
       * all the processors gives the set of indices of the distributed vector.
       * However, each processor also needs information on a set of non-owned
       * indices (hereafter termed ghost indices) based on the needs of the
       * application. Based on the ghost indices, the current processor can
       * easily determine the processor where it is owned. These processors
       * are termed as target processors to which the current processor
       * has to send a request to access the ghost data. Similarly, the ghost
       * indices in some other processor might be owned by this processor. In
       * that case, the other processor will be sending a request to the current
       * processor to access some of its data (data which is ghost to the other
       * processor but owned by the current processor). But the current
       * processor has no apriori knowledge of which processors will be
       * requesting data from it. A knowledge of it will help the current
       * processor to prepare for the request of data.
       *
       ...
       * source is the index of the source processor sending the message and tag
       * is an MPI_tag associated with exchange. It does not initiate any
       ...
       * MPI_ANY_SOURCE and MPI_ANY_TAG, as we just want to know if there is an
       * incoming message or not. In the event that there is an incoming message
       * (i.e., the MPI_IProbe's flag is true), we can extract the source
       * processor from the status handle of the MPI_IProbe and append it to
       * a list that stores the requesting processor IDs. Addtionally, in
       * the event that there is an incoming messag, we call a non-blocking
       ...
       *     of all its "local-send").
       * (b) the above only informs the network that the all "local-send" of the
       *     current processor have been received. But the current processor can
       *     still have incoming messages to be receieved. Hence, the current
