    ...
    namespace blasLapack
    {
       ...
       * @brief Standard hermitian eigenvalue decomposition
          ...
          [in]	range	
               ...
               For range = Value or Index and iu - il < n - 1, lapack::stebz and lapack::stein are called.
          ...
          [in]	abstol	The absolute error tolerance for the eigenvalues. An approximate eigenvalue is accepted as converged when it is determined to lie in an interval [a,b] of width less than or equal to
          abstol + eps * max( |a|,|b| ),
          ...
          Doing so will guarantee that eigenvalues are computed to high relative accuracy when possible in future releases. 
          The current code does not make any guarantees about high relative accuracy, but future releases will. 
          ...
          [out]isuppz	The vector isuppz of length 2*max(1,nfound). The support of the eigenvectors in Z, i.e., the indices indicating the nonzero elements in Z. 
               The i-th eigenvector is nonzero only in elements isuppz( 2*i-1 ) through isuppz( 2*i ).
                This is an output of lapack::stemr (tridiagonal matrix).
                 The support of the eigenvectors of A is typically 1:n because of the unitary transformations applied by lapack::unmtr. 
