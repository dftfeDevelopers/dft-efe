...
namespace dftefeScalapack
{
    ...
    const dftefeScalapack::LAPACKSupport::Property property_)
  {
    ...
    state             = dftefeScalapack::LAPACKSupport::State::matrix;
    property          = property_;
    grid              = process_grid;
    n_rows            = n_rows_;
  ...
  template <typename NumberType>
  dftefeScalapack::LAPACKSupport::Property
  ...
  template <typename NumberType>
  dftefeScalapack::LAPACKSupport::State
                                   ...
                                   const bool transpose_B)
  {
    ...
    state = dftefeScalapack::LAPACKSupport::matrix;
  }
                                    ...
                                    const bool transpose_B) const
  {
    ...
    C.state = dftefeScalapack::LAPACKSupport::matrix;
  }
  ...
  ScaLAPACKMatrix<NumberType>::compute_cholesky_factorization()
  {
    ...
    property = (uplo == 'L' ? dftefeScalapack::LAPACKSupport::lower_triangular :
                              dftefeScalapack::LAPACKSupport::upper_triangular);
  }
  ...
  ScaLAPACKMatrix<NumberType>::invert()
  {
    ...
    const bool is_triangular =
      (property == dftefeScalapack::LAPACKSupport::upper_triangular ||
       property == dftefeScalapack::LAPACKSupport::lower_triangular) &&
      (state == dftefeScalapack::LAPACKSupport::State::matrix ||
       state == dftefeScalapack::LAPACKSupport::State::inverse_matrix);
    ...
    if (is_triangular)
    ...
    else
      {
        ...
        // matrix.
        if (!(state == dftefeScalapack::LAPACKSupport::State::lu ||
              state == dftefeScalapack::LAPACKSupport::State::cholesky))
          {
            if (is_hermitian)
  ...
  {
    Assert(state == dftefeScalapack::LAPACKSupport::matrix,
           dealii::ExcMessage(
             "Matrix has to be in Matrix state before calling this function."));
    Assert(property == dftefeScalapack::LAPACKSupport::hermitian,
           dealii::ExcMessage(
    ...
    if (compute_eigenvectors)
      {
        property = dftefeScalapack::LAPACKSupport::Property::general;
        state    = dftefeScalapack::LAPACKSupport::eigenvalues;
      }
    else
      state = dftefeScalapack::LAPACKSupport::unusable;
