...
namespace dftefe
{
  namespace linearAlgebra
  {
    /*   template <typename ValueType, dftefe::utils::MemorySpace memorySpace>
       Matrix<ValueType, memorySpace>::Matrix(size_type rows, size_type cols,
       MPI_Comm comm, int64_t p, int64_t q, int64_t nb) : d_comm(comm), d_p(p),
       d_q(q), d_nb(nb), d_nGlobalRows(rows), d_nGlobalCols(cols) {
         ...
         const Matrix<ValueType, memorySpace> &rhs)
       {
         utils::throwException<utils::LengthError>(
           ...
           "Mismatch of global sizes of the two Matrices that are being
       added."); utils::throwException<utils::LengthError>( (rhs.getLocalRows()
       == this->getLocalRows()) && (rhs.getLocalCols() == this->getLocalCols()),
