...
namespace dftefe
{
  namespace linearAlgebra
  {
	...
	template <typename ValueTypeOperator, typename ValueTypeOperand, utils::MemorySpace memorySpace>
	  class PreconditionerJacobi: public Preconditioner<ValueTypeOperator, ValueTypeOperand, ValueTypeOperand>
	{
	  public:
		 ...
		 *
		 * @param[in] diagonal MemoryStorage object containing the local part of diagonal of the matrix. The local part denotes the union of the 
		 * locally owned and the ghost part of the matrix (i.e, the locally relevant part)
		 *
		 * @tparam memorySpaceSrc The memory space in which the input parameter diagonal resides. This allows
		 * us to seamlessly transfer data from any memory space (i.e., memorySpaceSrc) to the memory space
		 * of the PreconditionerJacobi object (i.e., memorySpace)
		 *
		 */
		template<utils::MemorySpace memorySpaceSrc>
		  PreconditionerJacobi(const utils::MemoryStorage<ValueTypeOperator, memoryStorageSrc> & diagonal);
		
		/**
		 * @brief Constructor based on input MemoryStorage
		 *
		 * @param[in] diagonal Vector containing diagonal of the matrix. 
		 *
		 * @tparam memorySpaceSrc The memory space in which the input parameter diagonal resides. This allows
		 * us to seamlessly transfer data from any memory space (i.e., memorySpaceSrc) to the memory space
		 * of the PreconditionerJacobi object (i.e., memorySpace)
		 ...
		 */
		template<utils::MemorySpace memorySpaceSrc>
		  PreconditionerJacobi(const Vector<ValueTypeOperator, memoryStorageSrc> & diagonal);

		/**
		 * @brief Constructor based on input pointer
		 *
		 * @param[in] diagData Pointer to data containing the local part of diagonal of the matrix. 
		 * The local part denotes the union of the locally owned and the ghost part of the matrix (i.e, the locally relevant part)
		 *
		 * @param[in] N size of the local part of the diagonal of the matrix (..e, the size of the locally owned plus the ghost indices)
		 *
		 * @note: This constructor assumes that the data pointed by diagData resides in the memory space of the PreconditionerJacobi
		 * (i.e., memorySpace). 
		 *
		...
		void
		  apply(const Vector<ValueTypeOperand, memorySpace> & x,
			  Vector<scalar_type<ValueTypeOperator, ValueTypeOperand>, memorySpace> & y) const override;

		void
		  apply(const MultiVector<ValueTypeOperand, memorySpace> & X,
			  MultiVector<scalar_type<ValueTypeOperator, ValueTypeOperand>, memorySpace> & Y) const override;
