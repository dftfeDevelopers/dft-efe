  ...
  namespace linearAlgebra
  {
...

    {
      ...
      std::string msg = "Error occured while using MPI_Comm_rank. "
               "Error code: " +
               std::to_string(err);
      throwException(err == MPI_SUCCESS, msg);
      if(mpiRank == 0)
      {
	msg = "WARNING: Constructing a DistributedVector using locally owned "
	...
	std::cout << msg << endl;
      ...
      d_mpiPatternP2P = std::make_shared<const MPIPatternP2P>(
	locallyOwnedRange,
        ghostIndices,
        mpiComm);
      ...
      d_mpiCommunicatorP2P = std::make_shared<const MPICommunicatorP2P>(
      ...
      d_mpiPatternP2P(u.d_mpiPatternP2P)
    {
      ...
      bool areCompatible =
        d_vectorAttributes.areDistributionCompatible(vectorAttributesDistributed);
	...
	VectorKernels<ValueType, memorySpace>::l2Norm(d_localOwnedSize,
                                                      this->data());
      const double l2NormLocallyOwnedSquare = l2NormLocallyOwned*l2NormLocallyOwned; 
    ...
    template <typename ValueType, dftefe::utils::MemorySpace memorySpace>
	 ...
	 const size_type communicationChannel /*= 0*/) 
     {
       //d_mpiCommunicatorP2P->scatterToGhostBegin(d_storage, communicationChannel);
     }
    ...
    template <typename ValueType, dftefe::utils::MemorySpace memorySpace>
	 ...
	 const size_type communicationChannel /*= 0*/) 
     {
       //d_mpiCommunicatorP2P->scatterToGhostEnd(d_storage, communicationChannel);
     }
    ...
    template <typename ValueType, dftefe::utils::MemorySpace memorySpace>
	 ...
	 const size_type communicationChannel /*= 0*/) 
     {
       //d_mpiCommunicatorP2P->gatherFromGhostBegin(d_storage, communicationChannel);
     }
    ...
    template <typename ValueType, dftefe::utils::MemorySpace memorySpace>
	 ...
	 const size_type communicationChannel /*= 0*/) 
     {
       //d_mpiCommunicatorP2P->gatherFromGhostEnd(d_storage, communicationChannel);
     }
