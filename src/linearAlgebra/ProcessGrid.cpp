...
namespace dftefe
{
  namespace linearAlgebra
  {
    namespace
    {
      ...
      compute_processor_grid_sizes(const utils::mpi::MPIComm &mpi_comm,
                                   ...
                                   const unsigned int         block_size_n)
      {
        ...
        // processes available: Pr x Pc <= P This, however, depends on the task
        // to be done. LU , QR and QL factorizations perform better for “flat”
        ...
        // one could rounds up Pc to the number which has zero remainder from
        // the division of Np while ( Np % Pc != 0 )
        //  ++Pc;
        // but this affects the grid shape dramatically, i.e. 10 cores 3x3
        // becomes 2x5.
      ...
      , n_mpi_processes(utils::mpi::numMPIProcesses(mpi_communicator))
    {
      ...
      // rank 0) the resulting utils::mpi::MPIComm
      // mpi_communicator_inactive_with_root will be utils::mpi::MPICommNull.
      const int mpi_tag = static_cast<int>(
        utils::mpi::MPITags::SCALAPACK_PROCESS_GRID_CONSTRUCTOR);

      ierr =
        utils::mpi::MPICommCreateGroup(mpi_communicator,
                                       inactive_with_root_group,
                                       mpi_tag,
      ...
      if (mpi_communicator_inactive_with_root != utils::mpi::MPICommNull &&
          utils::mpi::thisMPIProcess(mpi_communicator_inactive_with_root) == 0)
        DFTEFE_Assert(mpi_process_is_active);
#endif
    }
    ...
    ProcessGrid::ProcessGrid(const utils::mpi::MPIComm &mpi_comm,
    ...
    ProcessGrid::send_to_inactive(NumberType *value, const int count) const
    {
      ...
      if (mpi_communicator_inactive_with_root != utils::mpi::MPICommNull)
        {
          const int ierr = utils::mpi::MPIBcast<utils::MemorySpace::HOST>(
            value,
            count,
    ...
    template void
    ProcessGrid::send_to_inactive<std::complex<double>>(std::complex<double> *,
                                                        const int) const;
  } // namespace linearAlgebra
} // namespace dftefe
