...
namespace dftefe
{
    ...
    const unsigned int loc_column) const
  {
    ...
    return indxl2g_(&j,
                    ...
                    &(grid->get_this_process_column()),
                    &first_process_column,
                    &(grid->get_process_grid_columns())) - 1;
  }
  ...
  ScaLAPACKMatrix<NumberType>::copy_to(ScaLAPACKMatrix<NumberType> &dest) const
  {
    ...
    if ((this->grid != dest.grid) || (row_block_size != dest.row_block_size) ||
        (column_block_size != dest.column_block_size))
      {
        ...
        ierr = utils::mpi::MPICommGroup(this->grid->get_mpi_communicator(), &group_source);
        AssertThrowMPI(ierr);
        ierr = utils::mpi::MPICommGroup(dest.grid->get_mpi_communicator(), &group_dest);
        AssertThrowMPI(ierr);
                                    ...
                                    const bool transpose_B) const
  {
    DFTEFE_AssertWithMsg(this->grid == B.grid,
             "The matrices A and B need to have the same process grid");
    DFTEFE_AssertWithMsg(C.grid == B.grid,
  ...
  ScaLAPACKMatrix<NumberType>::compute_cholesky_factorization()
  {
    ...
    DFTEFE_AssertWithMsg(state == LAPACKSupport::matrix,
    ...
    std::unique_ptr<ScaLAPACKMatrix<NumberType>> eigenvectors =
      compute_eigenvectors ?
        ...
        std::make_unique<ScaLAPACKMatrix<NumberType>>(
          grid->get_process_grid_rows(), grid->get_process_grid_columns(), grid, 1, 1);
    ...
    if (grid->is_process_active())
      {
        ...
        if (all_eigenpairs)
        ...
        else
          {
            ...
            abstol *= 2;
            ifail.resize(n_rows);
            iclustr.resize(2 * grid->get_process_grid_rows() * grid->get_process_grid_columns());
            gap.resize(grid->get_process_grid_rows() * grid->get_process_grid_columns());
