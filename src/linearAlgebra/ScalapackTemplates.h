...
namespace dftefe
{
  namespace linearAlgebra
  {
    extern "C"
    {
      ...
      /**
       * Determine how many processes are available and the current process
       * rank.
       *
       ...
       * into the process grid. Input arguments must be the same on every
       * process.
       *
      ...
      /**
       * Given the system process number, return the row and column coordinates
       * in the BLACS' process grid.
       */
      void
       ...
       */
      int
      Csys2blacs_handle(utils::mpi::MPIComm comm);
       ...
       * @f}
       * where
       * $\mathrm{sub}(C)$ denotes C(IC:IC+M-1,JC:JC+N-1),  and, $op(X)$ is one
       * of $op(X) = X$ or $op(X) = X^T$.
       */
      void
      ...
      /**
       * Return the value of the one norm, or the Frobenius norm, or the
       ...
       * routines. In its present form, the routine assumes a homogeneous system
       * and makes no checks for consistency of the eigenvalues or eigenvectors
       ...
       *  of a real symmetric matrix A using a parallel implementation of the
       * MRR algorithm. Eigenvalues/vectors can be selected by specifying a
            ...
            int /*csrc*/)
    {
      utils::throwException(false,
                            "Function in ScalapackTemplates.h not implemented");
    }
