...
namespace dftefe
{
  namespace ksdft
  {
    ...
    ElectrostaticAllElectronFE<ValueTypeBasisData,
      ...
      ElectrostaticAllElectronFE(
        const basis::FEBasisDataStorage<ValueTypeBasisData, memorySpace>
          &feBasisDataStorage, const size_type maxCellTimesNumVecs)
        ...
        std::shared_ptr<linearAlgebra::LinAlgOpContext<memorySpace>> linAlgOpContext)
    {
      ...
      std::shared_ptr<dftefe::linearAlgebra::MultiVector<ValueTypeBasisData, memorySpace>>
        solution;

      std::shared_ptr<dftefe::linearAlgebra::LinearSolverFunction<ValueTypeBasisData,
                                                      ValueTypeBasisCoeff,
                                                      memorySpace>> linearSolverFunction =
      std::make_shared<dftefe::electrostatics::PoissonLinearSolverFunctionFE<ValueTypeBasisData,
                                                      ValueTypeBasisCoeff,
                                                      memorySpace,
                                                      dim>>
                                                      (feBasisManager,
      ...
      std::shared_ptr<dftefe::linearAlgebra::LinearSolverImpl<ValueTypeBasisData,
                                                      ValueTypeBasisCoeff,
                                                      memorySpace>> CGSolve =
      std::make_shared<dftefe::linearAlgebra::CGLinearSolver<ValueTypeBasisData,
                                                      ValueTypeBasisCoeff,
                                                      memorySpace>>
                                                      ( maxIter,
                                                      absoluteTol,
                               ...
                               dim>::evalEnergy() const
    {
      ...
      for (unsigned int i = 0 ; i < numQuadraturePoints ; i++ )
      {
        ...
        {
          e += *(i*numComponents+j+iter1) * *(i*numComponents+j+iter2) 
                * JxW[i] * 0.5;
        }
      }
      energy[iProb] = e;
