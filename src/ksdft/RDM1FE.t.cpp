...
namespace dftefe
{
  namespace ksdft
  {
      ...
      , d_waveFuncBatchSize(waveFuncBatchSize)
    {
      ...
      d_densCalc =
        std::make_shared<DensityCalculator<ValueTypeBasisData,
                                           ValueTypeBasisCoeff,
                                           memorySpace,
    ...
    template <typename ValueTypeBasisData,
              ...
              size_type dim>
    bool
    RDM1FE<ValueTypeBasisData, ValueTypeBasisCoeff, memorySpace, dim>::isSpinPolarized() const
    {
    ...
    template <typename ValueTypeBasisData,
              ...
              size_type dim>
    bool
    RDM1FE<ValueTypeBasisData, ValueTypeBasisCoeff, memorySpace, dim>::isNonCollinear() const
    {
    ...
    template <typename ValueTypeBasisData,
              ...
              size_type dim>
    bool
    RDM1FE<ValueTypeBasisData, ValueTypeBasisCoeff, memorySpace, dim>::isSOC() const
    {
      return d_isSOC;
    }
    ...
    template <typename ValueTypeBasisData,
              ...
              size_type dim>
    size_type
    RDM1FE<ValueTypeBasisData, ValueTypeBasisCoeff, memorySpace, dim>::getnKSOrbs() const
    {
    ...
    template <typename ValueTypeBasisData,
              ...
              size_type dim>
    std::vector<double>
    RDM1FE<ValueTypeBasisData, ValueTypeBasisCoeff, memorySpace, dim>::getkPointCoords() const
    {
    ...
    template <typename ValueTypeBasisData,
              ...
              size_type dim>
    std::vector<double>
    RDM1FE<ValueTypeBasisData, ValueTypeBasisCoeff, memorySpace, dim>::getkPointWeights() const
    {
    ...
    template <typename ValueTypeBasisData,
              ...
              size_type dim>
    void
    RDM1FE<ValueTypeBasisData, ValueTypeBasisCoeff, memorySpace, dim>::setEvalDescrFlag(
      const bool evalFlag)
    {
    ...
    template <typename ValueTypeBasisData,
              ...
              size_type dim>
    void
    RDM1FE<ValueTypeBasisData, ValueTypeBasisCoeff, memorySpace, dim>::getDescriptors(
        ...
        std::unordered_map<WfcDescrAttr, AttrStorage> & wfcAttrVals)
    {
      ...
      if (needEval)
      {
        dftefe::utils::throwException<dftefe::utils::LogicError>(this->d_ksSetFlag,
        ...
        size_type ncomp = 1;
        if (d_isSpinPolarized)  ncomp = 2;
        else if (d_isNonCollinear) ncomp = 4;

        // Use the first k-point/spin occupancy for density computation.
        // For non-spinpolarized single-k-point: d_occupancies[0] has length nKSOrbs.
        const std::vector<double> &occ =
          (this->d_occupancies.empty() ? std::vector<double>{} : this->d_occupancies[0]);
    ...
    std::unique_ptr<RDM1<
      linearAlgebra::blasLapack::scalar_type<ValueTypeBasisData,
                                             ValueTypeBasisCoeff>,
      memorySpace>>
    RDM1FE<ValueTypeBasisData, ValueTypeBasisCoeff, memorySpace, dim>::clone() const
    {
    ...
    template <typename ValueTypeBasisData,
              ...
              size_type dim>
    void
    RDM1FE<ValueTypeBasisData, ValueTypeBasisCoeff, memorySpace, dim>::getDensityObs(
        const std::set<DensityObsAttr> & densityObsAttrs,
        std::unordered_map<DensityObsAttr, std::vector<std::vector<double>>> & densityObsAttrVals)
    {
      size_type ncomp = 1;
      if (d_isSpinPolarized)   ncomp = 2;
      else if (d_isNonCollinear) ncomp = 4;
      ...
      const bool needDensityEval = this->d_evalFlag ||
        (d_densityAttrVals.find(DensityDescrAttr::Val) == d_densityAttrVals.end());
      if (needDensityEval)
      {
      ...
      {
        if (attr == DensityObsAttr::Dipole     && maxMoment < 1) maxMoment = 1;
        if (attr == DensityObsAttr::Quadrupole && maxMoment < 2) maxMoment = 2;
      }
      ...
      for (size_type iComp = 0; iComp < ncomp; ++iComp)
      {
        ...
        if (cappedMaxMoment >= 1) nMoments += 3;
        if (cappedMaxMoment >= 2) nMoments += 9;
        ...
        if (cappedMaxMoment >= 2)
        {
          ...
          moments[4]=Qxx; moments[5]=Qxy; moments[6]=Qxz;
          moments[7]=Qxy; moments[8]=Qyy; moments[9]=Qyz;
          moments[10]=Qxz; moments[11]=Qyz; moments[12]=Qzz;
        }
