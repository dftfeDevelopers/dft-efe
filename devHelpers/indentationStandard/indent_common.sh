...
#
# DFTEFE_II_CLANG_FORMAT can be used to change the default version of
# clang-format
#

export DFTEFE_II_CLANG_FORMAT="${DFTEFE_II_CLANG_FORMAT:-clang-format}"

checks() {
  #if test ! -d source -o ! -d include -o ! -d examples ; then
  #  echo "*** This script must be run from the top-level directory of dft-efe."
  #  exit 1
  #fi

  # Add the location 'download_clang_format'
  ...
  if ! [ -x "$(command -v "${DFTEFE_II_CLANG_FORMAT}")" ]; then
    echo "***   No clang-format program found."
    echo "***"
    echo "***   You can run the './devhelpers/utilities/download_clang_format'"
    echo "***   to install a compatible binary into './devhelpers/utilities/programs'."
    exit 1
  fi

  # Make sure to have the right version.
  CLANG_FORMAT_VERSION="$(${DFTEFE_II_CLANG_FORMAT} --version)"
...
format_file()
{
  ...
  "${DFTEFE_II_CLANG_FORMAT}" "${file}" > "${tmpfile}"
...
format_inst()
{
  ...
  # directory.
  #
  "${DFTEFE_II_CLANG_FORMAT}" < "${tmpfile}" > "${tmpfile}new"
