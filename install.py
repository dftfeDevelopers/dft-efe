...
def getUsageMsg():
    msg = '''
    The correct usage is python install.py [--build_dir=/path/to/build/directory]
    The optional [--build_dir=/path/to/build/directory] specifies where to build the dft-efe executable. 
    By default it creates a build directory inside the dft-efe parent directory'''
    return msg
